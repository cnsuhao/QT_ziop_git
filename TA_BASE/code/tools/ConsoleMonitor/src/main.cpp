...
int main(int argc, char* argv[])
{
    ...
    if ( TA_Base_Core::RunParams::getInstance().isSet( "BatchMode" ) )
    {
        ...
        while ( true )
        {
            ...
            {
                TA_Base_Core::BaseCommandParser* parser = TA_Base_Core::CommandParserFactory::getInstance().getCurrentCommandParser();
                const char* currModuleName = parser->getModuleName();
...

    {
        ...
        TA_Base_Core::BaseCommandParser* currParser = TA_Base_Core::CommandParserFactory::getInstance().getCurrentCommandParser();
        assert(currParser != NULL);
        std::string endPoint = TA_Base_Core::RunParams::getInstance().get(RPARAM_NOTIFYHOSTS);
        assert( false == endPoint.empty() );

        currParser->initCorbaEntity("OccNotificationServiceAgent", endPoint);

/*
        //just for test
        TA_Base_Core::BaseCommandParser* currParser = TA_Base_Core::CommandParserFactory::getInstance().setCurrentCommandParser("common");
        assert(currParser != NULL);
