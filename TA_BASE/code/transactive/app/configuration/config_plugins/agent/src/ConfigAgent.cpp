 ...
 * in any form.
 *
 * Source: $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/configuration/config_plugins/agent/src/ConfigAgent.cpp $
...
namespace TA_Base_App
{
    ...
    TA_Base_Core::ItemChanges ConfigAgent::getAllItemChanges()
    {
        ...
        if (matching != changes.end())
        {
            ...
            int pos = name.find_last_of(MONITORED_PROCESS_NAME_APPEND);
            if (pos != -1)
            {
    ...
    void ConfigAgent::applyChanges()
    {
        ...
        catch( const TA_Base_Core::DataException& exception)
        {
                 ...
                 (exception.getFailType() == TA_Base_Core::DataException::WRONG_TYPE) )
            {
                ...
                int pos = invalidFields.find(ADDRESS_FIELD_NAME);
                if (pos != -1)
                {
