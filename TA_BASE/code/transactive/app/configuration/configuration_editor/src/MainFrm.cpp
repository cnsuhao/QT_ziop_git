/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/configuration/configuration_editor/src/MainFrm.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
	...
	std::string CMainFrame::WriteTDSLog(int dbType)
	{
		...
		else
		{
			...
			try
			{
				...
				tdsRecord.push_back(sqlColName);
				...
				databaseConnection->prepareSQLStatement(sql, TDS_SQLLOG_SELECT_81002, sqlColName);
				...
				{
					LOG_GENERIC(SourceInfo,TA_Base_Core::DebugUtil::DebugError,"Table TDS_SQLLOG not has 1 row!");
				}
				else
				{
					str += data->getStringData(0, sqlColName);
				}
