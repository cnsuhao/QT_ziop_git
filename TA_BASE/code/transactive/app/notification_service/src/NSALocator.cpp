 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/notification_service/src/NSALocator.cpp $
 * @author:  Jade Welton
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_App
{
	...
	void NSALocator::initNSALocator()
	{
        ...
        public:
            ...
            static std::vector<unsigned long>  getPhysicalConnectedLocationList()
            {
                std::vector<unsigned long> locations = getNearbyLocationList();
                unsigned long occLocationKey = TA_Base_Core::LocationAccessFactory::getInstance().getOccLocationKey();
                locations.push_back( occLocationKey );
                return locations;
            }

        private:

            static bool isStation(const std::string& stationName)
            {
                static std::set<std::string> specialStationSet;

                if ( true == specialStationSet.empty() )
                {
                    specialStationSet.insert( "OCC" );
                    specialStationSet.insert( "TDS" );
                    specialStationSet.insert( "KCD" );
                    specialStationSet.insert( "DCC" );
                    specialStationSet.insert( "KOCC" );
                    specialStationSet.insert( "TMF" );
                }

                return ( specialStationSet.find( stationName ) == specialStationSet.end() );
            }

            static std::vector<unsigned long>  getNearbyLocationList()
            {
                std::vector<unsigned long> nearbyLocationList;
                std::vector<unsigned long> locationOrderIdList;
                std::vector<ILocation*> locations = TA_Base_Core::LocationAccessFactory::getInstance().getAllLocations();

                for ( size_t i = 0; i < locations.size(); ++i )
                {
                    if ( locations[i] != NULL && locations[i]->getKey() != 0 && isStation( locations[i]->getName() ) )
                    {
                        locationOrderIdList.push_back( locations[i]->getOrderId() );
                    }
                }

                std::sort( locationOrderIdList.begin(), locationOrderIdList.end() );

                if ( locationOrderIdList.empty() )
                {
                    return std::vector<unsigned long>();
                }

                unsigned long minLocationOrderId = locationOrderIdList.front();
                unsigned long maxLocationOrderId = locationOrderIdList.back();

                LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                    "NSALocator::LocationHelper::getNearbyLocationList(): minLocationOrderId=%d, maxLocationOrderId=%d", minLocationOrderId, maxLocationOrderId);

                unsigned long locationKey = TA_Base_Core::gGetLocalLocationKey();
                boost::shared_ptr<TA_Base_Core::ILocation> currentLocation( TA_Base_Core::LocationAccessFactory::getInstance().getLocationByKey(locationKey));

                for ( size_t i = 0; i < locations.size(); ++i )
                {
                    if ( true == isNearbyLocation( locations[i], currentLocation.get(), minLocationOrderId, maxLocationOrderId ) )
                    {
                        nearbyLocationList.push_back( locations[i]->getKey() );
                        LOG2( SourceInfo, TA_Base_Core::DebugUtil::DebugDebug,
                            "NSALocator::LocationHelper::getNearbyLocationList(): add nearby location: %s %d", locations[i]->getName().c_str(), locations[i]->getKey());
                    }
            ...
            static bool isNearbyLocation(TA_Base_Core::ILocation* location, TA_Base_Core::ILocation* currentLocation, unsigned long minLocationOrderId, unsigned long maxLocationOrderId)
            {
                if ( NULL == location || NULL == currentLocation )
                {
                    return false;
                }

                static bool isCircleLocation = TA_Base_Core::RunParams::getInstance().isSet( "CircleLocation" );

                unsigned long locationOrderId = location->getOrderId();
                unsigned long currentLocationOrderId = currentLocation->getOrderId();

                if ( currentLocationOrderId == minLocationOrderId )
                {
                    if ( ( locationOrderId == minLocationOrderId ) || ( locationOrderId == ( minLocationOrderId + 1 ) ) )
                    {
                        return true;
                    }

                    if ( true == isCircleLocation && locationOrderId == maxLocationOrderId )
                    {
                        return true;
                    }
                }
                else if ( currentLocationOrderId == maxLocationOrderId )
                {
                    if ( ( locationOrderId == maxLocationOrderId ) || ( locationOrderId == ( maxLocationOrderId - 1) ) )
                    {
                        return true;
                    }

                    if ( true == isCircleLocation && locationOrderId == minLocationOrderId )
                    {
                        return true;
                    }
                }
                ...
                {
                    int locationDistance = std::abs( (int)locationOrderId - (int)currentLocationOrderId );
                ...
                if ( boost::iequals( currentLocation->getName(), "DT1" ) || boost::iequals( currentLocation->getName(), "DT16" ) )
                {
		...
		for (std::vector< ILocation* >::iterator iter = locations.begin(); iter != locations.end(); ++iter)
		{
			if (*iter != NULL)
			{
				try
				{
					...
					{
                        const unsigned long locationKey = TA_Base_Core::gGetLocalLocationKey();
                        const unsigned long occLocationKey = TA_Base_Core::LocationAccessFactory::getInstance().getOccLocationKey();

                        if ( locationKey == occLocationKey  )
                        {
                        ...
                        else
                        {
                            entities = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfTypeAtLocation(
                                NotificationAgentData::getStaticType(), LocationHelper::getPhysicalConnectedLocationList(), false);
                        }
					}
					else
					{
