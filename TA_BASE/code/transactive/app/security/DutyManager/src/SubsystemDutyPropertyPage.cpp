/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/security/DutyManager/src/SubsystemDutyPropertyPage.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
//IMPLEMENT_DYNCREATE(SubsystemDutyPropertyPage, AbstractPropertyPage)

SubsystemDutyPropertyPage::SubsystemDutyPropertyPage(ITransActiveWinApp * pWinApp, UINT nIDTemplate, UINT nIDCaption  ) :
TA_Base_Bus::AbstractPropertyPage(pWinApp, SubsystemDutyPropertyPage::IDD, nIDCaption),
m_realPModel(NULL)
{ 
...
std::string SubsystemDutyPropertyPage::getMyPModelName()
{
	return PMODEL_SUBSYSTEM_DUTY_PROPERTY_PAGE;
}

void SubsystemDutyPropertyPage::setupPModel()
{
	m_realPModel = dynamic_cast<SubsystemDutyPropertyPageModel*> (m_pModel);
	TA_ASSERT(m_realPModel != NULL, "PModel should be valid");

}

void SubsystemDutyPropertyPage::initUIControl() 
{
	enableDutyControlButtons();
}

TA_Base_Bus::DataCache& SubsystemDutyPropertyPage::getDataCache()
{
	return m_realPModel->getDataCache();
}

TA_Base_Bus::SessionCache& SubsystemDutyPropertyPage::getSessionCashe()
{
	return m_realPModel->getSessionCashe();
}


TA_Base_Bus::SubsystemDutyMatrix& SubsystemDutyPropertyPage::getSubsystemDutyMatrix()
{
	return m_realPModel->getSubsystemDutyMatrix();
}

TA_Base_Bus::SessionInfoEx  SubsystemDutyPropertyPage::getCurrentSessionInfo()
{
	return m_realPModel->getCurrentSessionInfo();
}

TA_Base_Bus::ProfileKeyList	SubsystemDutyPropertyPage::getPreviousProfileId()
{
	return m_realPModel->getPreviousProfileId();
}

bool SubsystemDutyPropertyPage::canTransferSubsystems(long lActionId)
{
	return m_realPModel->canTransferSubsystems(lActionId);
}

std::string SubsystemDutyPropertyPage::getRegionDisplayName(RegionKey key)
{
	return m_realPModel->getRegionDisplayName(key);
	// m_dataCache.getRegionDisplayName(*regionIt).c_str();
}

std::string SubsystemDutyPropertyPage::getRegionName(RegionKey key)
{
...
void SubsystemDutyPropertyPage::notifyUpdate(const char * updateType, TA_Base_Bus::EventArgs * args)
{
}

void SubsystemDutyPropertyPage::requestUserReply(TA_Base_Bus::UIReplyEventArgsPtr uiEvent)
{
...
void SubsystemDutyPropertyPage::initialiseSubsystemDutyGrid()
{
    ...
    m_normalDutyColour = convertColourStrToColorref( m_realPModel->getNormalDutyColour(),
                                                     NORMAL_DUTY_COLOUR );
    m_delegatedDutyColour = convertColourStrToColorref( m_realPModel->getDelegatedDutyColour(),
                                                     DELEGATED_DUTY_COLOUR );
    m_degradedDutyColour = convertColourStrToColorref( m_realPModel->getDegradedDutyColour(),
                                                     DEGRADED_DUTY_COLOUR );
    m_unallocatedSubsystemColour = convertColourStrToColorref( m_realPModel->getUnallocatedSubsystemColour(),
                                                     UNALLOCATED_SUBSYSTEM_COLOUR );

    // By default only those subsystems are shown which the session currently has access to.
    SubsystemKeyList subsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(getCurrentSessionInfo().ProfileId));
    RegionKeyList regionKeys(getSubsystemDutyMatrix().getColumnIds());
    ...
    for (RegionKeyList::const_iterator regionIt = regionKeys.begin(); regionIt != regionKeys.end(); regionIt++)
    {
        ...
        try
        {
            ...
            gridItem.col = column;
            gridItem.szText = getRegionDisplayName(*regionIt).c_str();
            gridItem.lParam = *regionIt;

			regionName = getRegionName(*regionIt).c_str();

			if(0 == regionName.CompareNoCase(All_LocationsStr))
			{
        ...
        catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
        {
            ...
            MessageBox::error("Error while adding region to subsystem duty grid.", ex.what(),"");
        }
    }
	...
	while(retryTime < CHECK_SESSION_IN_DB_RETRY_TIMES)
	{
		...
		{
			++retryTime;
			regionKeysAccessible = m_realPModel->getLocations(); 
			break;
		}
		catch (...)
		{
    ...
    {
        addSubsystem(*subsystemIt, getCurrentSessionInfo().ProfileId, regionKeysAccessible); //TD15533
    }
...
void SubsystemDutyPropertyPage::initialiseSubsystemDutyGridStatic()
{
	...
	for (RegionKeyList::const_iterator regionIt = regionKeys.begin(); regionIt != regionKeys.end(); regionIt++)
	{
		...
		catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
		{
			...
			MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0046).toMultiByteStdString(), ex.what(),"");
...
{
    bool bCanTransferSubsystems = canTransferSubsystems(aca_DUTY_TRANSFER_SUBSYSTEM);
	bool bCanTransferAllSubsystems = canTransferSubsystems(aca_DUTY_TRANSFER_ALL_SUBSYSTEM);

    m_transferSubsystemDutyBtn.EnableWindow(bCanTransferSubsystems);
    m_transferAllSubsystemDutyBtn.EnableWindow(bCanTransferAllSubsystems);
}
	...
	TA_Base_Bus::RegionKeyList regionKeys) //TD15533
{
    ...
    try
    {
        ...
        gridItem.szText = getDataCache().getSubsystemName(subsystemKey).c_str();
        ...
        for (gridItem.col = SUBSYSTEM_COLUMN + 1; gridItem.col < m_subsystemDutyGrid.GetColumnCount(); gridItem.col++)
        {
			...
			const SubsystemDutyMatrixElement& sdmElement = getSubsystemDutyMatrix().getElement(subsystemKey, regionKey);

            gridItem.mask = GVIF_PARAM;
            gridItem.lParam = (sdmElement.isSubsystemInRegion()? ALLOCATED_SUBSYSTEM : UNALLOCATED_SUBSYSTEM);

			
			if (!m_realPModel->isRegionAccessible(regionKey, regionKeys) ||  //requirement #4
            ...
            catch (const TA_Base_Core::MatrixException& ex)
            {
                ...
                //    << "subsystem " << getDataCache().getSubsystemName(subsystemKey)
                //    << " and region " << getDataCache().getRegionDisplayName(regionKey);

				TAUnicodeString tempStr = TAAfxFormatString2(STRING_ID_DUTY_MANAGER_0035,
					CONVERT_FROM_STD(getDataCache().getSubsystemName(subsystemKey)),
					CONVERT_FROM_STD(getDataCache().getRegionDisplayName(regionKey)));
                MessageBox::error(tempStr.toMultiByteStdString(), ex.what(),"");
            }
    ...
    catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
    {
        ...
        MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0026).toMultiByteStdString(), ex.what(),"");
    }
}
...
BOOL SubsystemDutyPropertyPage::OnInitDialog() 
{
	...
	AbstractPropertyPage::OnInitDialog();
...
void SubsystemDutyPropertyPage::OnSessionUpdate(const SessionUpdate& sessionUpdate)
{
    ...
    SubsystemKeyList previousSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights( getPreviousProfileId() ));
    SubsystemKeyList currentSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(getCurrentSessionInfo().ProfileId));
...
//    }
	...
	SubsystemKeyList empty;
	SubsystemKeyList allSubsystemKeys(getSubsystemDutyMatrix().getRowIds());

	if (m_showAllSubsystemsBtn.GetCheck() == BST_UNCHECKED)
	{
		updateSubsystems(previousSubsystemKeys, empty, getPreviousProfileId() );
		updateSubsystems(empty, currentSubsystemKeys, getCurrentSessionInfo().ProfileId);
	}
	else
	{
		updateSubsystems(allSubsystemKeys, empty, getCurrentSessionInfo().ProfileId);
        updateSubsystems(empty, allSubsystemKeys, getCurrentSessionInfo().ProfileId);
    }
	//++TD15533
...
void SubsystemDutyPropertyPage::OnSubsystemDutyUpdate(const SubsystemDutyUpdate& dutyUpdate)
{
    ...
    if ( ( TA_Base_Bus::DUT_ADD == dutyUpdate.updateType ) || ( TA_Base_Bus::DUT_REMOVE == dutyUpdate.updateType ) )
    {
        for (SubsystemDutyList::const_iterator subDutyIt = dutyUpdate.duties.begin(); subDutyIt != dutyUpdate.duties.end(); subDutyIt++)
        {
            try
            {
                ...
                if (getCurrentSessionInfo().SessionId != subDutyIt->session.in())
                {
                    continue;
                }
				...
				SubsystemDutyMatrixElement sdmElement = getSubsystemDutyMatrix().getElement(subDutyIt->subsystemKey, subDutyIt->regionKey);
				...
				if (!m_realPModel->isRegionAccessible(subDutyIt->regionKey, regionKeysAccessible) ||  
					(!isSubsystemAccessible(profileKeys, subDutyIt->subsystemKey) &&
					!m_realPModel->isRegionAccessible(subDutyIt->regionKey, regionKeysAccessible))|| 
					!sdmElement.isSubsystemInRegion()) 
					continue;
				//----TD19148
				...
				m_subsystemDutyGrid.SetItemText(row, column, g_string2CString(cellData.baseText.c_str()));
                m_subsystemDutyGrid.SetItemImageS(row, column, cellData.overrideImage);
				m_subsystemDutyGrid.SetItemTextS(row, column, g_string2CString(cellData.overrideText.c_str())); 
				...
				else*/ if ((cellData.baseText.compare("") == 0) && (cellData.overrideText.compare("") == 0))
				{
					...
					CString tempStr("");
					m_subsystemDutyGrid.SetItemImage(row, column, -1);
					m_subsystemDutyGrid.SetItemText(row, column, tempStr);
					m_subsystemDutyGrid.SetItemImageS(row, column, -1);
					m_subsystemDutyGrid.SetItemTextS(row, column, tempStr); 
					m_subsystemDutyGrid.SetItemOverrided(row, column, false); 
				}
    ...
    else if ( TA_Base_Bus::DUT_STATE == dutyUpdate.updateType )
    {
        for (SubsystemStateList::const_iterator subStateIt = dutyUpdate.states.begin(); subStateIt != dutyUpdate.states.end(); subStateIt++)
        {
            try
            {
				...
				SubsystemDutyMatrixElement sdmElement = getSubsystemDutyMatrix().getElement(subStateIt->subsystemKey, subStateIt->regionKey);
				COLORREF bkColour = determineCellBkColour( true, static_cast<TA_Base_Bus::ESubsystemState>(subStateIt->currState) );
				//TD19148---- LiDan 25,Jun,2007
				//2) Rights Manager of Local MFT Subsystem tab populate action group for other locations.

				if (!m_realPModel->isRegionAccessible(subStateIt->regionKey, regionKeysAccessible) ||  
					(!isSubsystemAccessible(profileKeys, subStateIt->subsystemKey) && 
					!m_realPModel->isRegionAccessible(subStateIt->regionKey, regionKeysAccessible)) || 
					!sdmElement.isSubsystemInRegion()) 
					continue;
				//----TD19148
...
void SubsystemDutyPropertyPage::OnShowAllSubsystems() 
{
    ...
    SubsystemKeyList allSubsystemKeys(getSubsystemDutyMatrix().getRowIds());
    SubsystemKeyList accessibleSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(getCurrentSessionInfo().ProfileId));
    ...
    if (m_showAllSubsystemsBtn.GetCheck() == BST_UNCHECKED)
    {
        updateSubsystems(allSubsystemKeys, accessibleSubsystemKeys, getCurrentSessionInfo().ProfileId); //TD15533
    }
    else
    {
        updateSubsystems(accessibleSubsystemKeys, allSubsystemKeys, getCurrentSessionInfo().ProfileId); //TD15533
    }
}
...
void SubsystemDutyPropertyPage::OnTransferSubsystemDutyBtn() 
{
    try
    {
		...
		{
			SubsystemDutyTransferDlg subDutyXferDlg(m_pWinApp);
		...
		else
		{
			...
			MessageBox::info(TALoadString(STRING_ID_DUTY_MANAGER_0040).toMultiByteStdString());
		}
    }
    catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
    {
        //MessageBox::error("Error while activating duty request dialog.", ex.what(),"");
		MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0047).toMultiByteStdString(), ex.what(),"");
    }
    catch (const TA_Base_Core::TransactiveException& ex)
    {
        //MessageBox::error("Error while activating duty request dialog.", ex.what(),"");
		MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0048).toMultiByteStdString(), ex.what(),"");
    }
	//catch (const TA_Base_Core::MatrixException& ex)
	//{
	//	MessageBox::error("Error while activating duty request dialog.", ex.what(),"");
    ...
    {
        MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0048).toMultiByteStdString());
    }	
}
...
void SubsystemDutyPropertyPage::OnTransferAllSubsystemDutyBtn() 
{
	...
	if ( TA_Base_App::DutyManagerDlg::m_isMartixDataInited == true )
	{
		if (userMsg.showMsgBox(IDS_UW_010003,TALoadString( STRING_ID_DUTY_MANAGER_0007 )) == IDYES) 
			// Are you sure you want to {0}? (where {0} = "request all subsystem duty")
		{
			try
			{
				...
				m_realPModel->transferAllSubsystemDuty();
			}
			catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
			{
				...
				{
					MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0049).toMultiByteStdString(), ex.what(),"");
				}
				else
				{
					MessageBox::warning(TALoadString(STRING_ID_DUTY_MANAGER_0050).toMultiByteStdString());
				}
	...
	else
	{
		//std::stringstream messageInfo;
		//messageInfo << "RightsManager is populating data, please try again later!";
		MessageBox::info(TALoadString(STRING_ID_DUTY_MANAGER_0051).toMultiByteStdString());
	}
	//++TD15569
}
	...
	const TA_Base_Bus::ESubsystemState subsystemState)
{
	...
	{
		if (getCurrentSessionInfo().ProfileId.size() == 2)
		{
			TA_Base_Bus::ActionGroupKey baseActionGroupKey = 
				m_realPModel->getActionGroupDB(getCurrentSessionInfo().ProfileId[0], subsystemKey, subsystemState);
			TA_Base_Bus::ActionGroupKey overrideActionGroupKey = 
				m_realPModel->getActionGroupDB(getCurrentSessionInfo().ProfileId[1], subsystemKey, subsystemState);
		...
		else if (getCurrentSessionInfo().ProfileId.size() == 1)
		{
			TA_Base_Bus::ActionGroupKey baseActionGroupKey = 
				m_realPModel->getActionGroupDB(getCurrentSessionInfo().ProfileId[0], subsystemKey, subsystemState);
	...
	else if (profilesWithDuties.size() == 1)
	{
		if (getCurrentSessionInfo().ProfileId.size() == 2)
		{
			if (profilesWithDuties[0].profileKey == getCurrentSessionInfo().ProfileId[0])
			{
				TA_Base_Bus::ActionGroupKey overrideActionGroupKey = 
					m_realPModel->getActionGroupDB(getCurrentSessionInfo().ProfileId[1], subsystemKey, subsystemState);
			...
			else if (profilesWithDuties[0].profileKey == getCurrentSessionInfo().ProfileId[1])
			{
				TA_Base_Bus::ActionGroupKey baseActionGroupKey = 
					m_realPModel->getActionGroupDB(getCurrentSessionInfo().ProfileId[0], subsystemKey, subsystemState);
	...
	if( getDataCache().getSubsystemName(subsystemKey).compare(TCS_TRAIN_COMMS_STN_SUBSYSTEM) == 0 // TCS subsystem
		&& m_realPModel->getProjectName().compare( TCS_SPECIAL_PROJECT_NAME ) == 0 // 4669 project
		&& subsystemState == TA_Base_Bus::E_SS_NORMAL // normal mode
		&& TA_Base_Core::SessionAccessFactory::getInstance().getSession( getCurrentSessionInfo().SessionId )->getLocationKey() == OCC_LOCATION // is OCC location
		)
	{
		TA_Base_Bus::SessionInfoEx sessionInfoEx = getSessionCashe().getSessionInfo( getCurrentSessionInfo().SessionId );
		...
		{
			if( getCurrentSessionInfo().ProfileId.size() == 1 ) // no override
			{
				cellData.baseImage = GridAssistant::GI_DELEGATE_TICK;
			}
			else if( getCurrentSessionInfo().ProfileId.size() == 2 ) // override
			{
				cellData.baseImage = GridAssistant::GI_DELEGATE_TICK;
		...
		else if( profilesWithDuties.size() == 1 && getCurrentSessionInfo().ProfileId.size() == 2 ) // override and only one profile has duty
		{
			if( profilesWithDuties[0].profileKey != getCurrentSessionInfo().ProfileId[0] ) // base profile has no duty
			{
				cellData.baseImage = GridAssistant::GI_DELEGATE_TICK;
			}
			else if( profilesWithDuties[0].profileKey != getCurrentSessionInfo().ProfileId[1] ) // override profile has no duty
			{
				cellData.overrideImage = GridAssistant::GI_DELEGATE_TICK;
			}
		}
	...
	const TA_Base_Bus::RegionKey regionKey)
{
    ...
    SubsystemDutyMatrixElement sdmElement = getSubsystemDutyMatrix().getElement(subsystemKey, regionKey);
    SubsystemDutyList subsystemDuties(sdmElement.getSubsystemDuties(getCurrentSessionInfo().SessionId));
	...
	const TA_Base_Bus::ActionGroupKey actionGroupKey)
{
	...
	{
		actionGroupName = getDataCache().getActionGroupName(actionGroupKey);
	}	
	...
	const TA_Base_Bus::SubsystemKey subsystemKey)
{
	...
	SubsystemKeyList accessibleSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(profileKeys));
