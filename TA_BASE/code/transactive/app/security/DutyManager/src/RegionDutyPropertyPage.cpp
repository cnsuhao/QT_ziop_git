/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/security/DutyManager/src/RegionDutyPropertyPage.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
//IMPLEMENT_DYNCREATE(RegionDutyPropertyPage, AbstractPropertyPage)
...
RegionDutyPropertyPage::RegionDutyPropertyPage(ITransActiveWinApp * pWinApp, UINT nIDTemplate, UINT nIDCaption ) :
TA_Base_Bus::AbstractPropertyPage(pWinApp, RegionDutyPropertyPage::IDD, nIDCaption),
m_delegateSubsystemDutyBtn(DELEGATE_SUB_DUTY_BTN_TXT_COLOUR, DELEGATE_SUB_DUTY_BTN_BK_COLOUR),
m_realPModel(NULL)
{
	FUNCTION_ENTRY("RegionDutyPropertyPage()");
...
std::string RegionDutyPropertyPage::getMyPModelName()
{
	 return PMODEL_REGION_DUTY_PROPERTY_PAGE;
}

void RegionDutyPropertyPage::setupPModel()
{
	m_realPModel = dynamic_cast<RegionDutyPropertyPageModel*> (m_pModel);
	TA_ASSERT(m_realPModel != NULL, "PModel should be valid");
	//m_realPModel->setProcesses(m_processes);
}

void RegionDutyPropertyPage::initUIControl()
{
	    enableDutyControlButtons();
}

TA_Base_Bus::DataCache& RegionDutyPropertyPage::getDataCache()
{
	return m_realPModel->getDataCache();
}

TA_Base_Bus::SessionCache& RegionDutyPropertyPage::getSessionCashe()
{
	return m_realPModel->getSessionCashe();
}


TA_Base_Bus::RegionDutyMatrix& RegionDutyPropertyPage::getRegionDutyMatrix()
{
	return m_realPModel->getRegionDutyMatrix();
}

std::string RegionDutyPropertyPage::getRegionDisplayName(RegionKey key)
{
	return m_realPModel->getRegionDisplayName(key);
	// m_dataCache.getRegionDisplayName(*regionIt).c_str();
}

std::string RegionDutyPropertyPage::getRegionName(RegionKey key)
{
	return m_realPModel->getRegionName(key);
	// m_dataCache.getRegionName(*regionIt).c_str();
}

void RegionDutyPropertyPage::notifyUpdate(const char * updateType, TA_Base_Bus::EventArgs * args)
{

}

void RegionDutyPropertyPage::requestUserReply(TA_Base_Bus::UIReplyEventArgsPtr uiEvent)
{
...
{
    SessionIdList sessionIds(getRegionDutyMatrix().getRowIds());
    RegionKeyList regionKeys(getRegionDutyMatrix().getColumnIds());
...
{
    m_transferRegionDutyBtn.EnableWindow(m_realPModel->canTransferRegions());
	// Due to [MW-181] on TAR0514_C955_Mysql_Win7_Issuelist.xls, need to hide the "Take Location Button"
	//std::string showSessionlocation = m_realPModel->getParam(RPARAM_SHOWSESSIONWORKSTATION).c_str();
	if ( false == m_realPModel->isShowSessionLocation() )
	{
		m_transferRegionDutyBtn.ShowWindow(SW_HIDE);
	}

    m_delegateSubsystemDutyBtn.EnableWindow(m_realPModel->canDegradeSubsystems());
}
...
void RegionDutyPropertyPage::addOperator(const TA_Base_Bus::SessionInfoEx& sessionInfoEx)
{
    ...
    for (int column = OPERATOR_COLUMN + 1; column < m_regionDutyGrid.GetColumnCount(); column++)
    {
        ...
        try
        {
			...
			CString profileCStr(profile.c_str());
            m_regionDutyGrid.SetItemText(row, column, profileCStr);
        }
        catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
        {
            std::ostringstream msg;
            msg << "Unable to initialise region duty grid cell for " << std::endl
                << "operator " << m_realPModel->getDataCache().getOperatorName(sessionInfoEx.UserId)
                << " and region " << m_realPModel->getDataCache().getRegionDisplayName(regionKey);
...
{
    addOperator(m_realPModel->getSessionInfo(sessionId));
}
...
void RegionDutyPropertyPage::updateOperator(int row, const TA_Base_Bus::SessionInfoEx& sessionInfoEx)
{
	...
	std::string text = m_realPModel->getOperatorDescription(sessionInfoEx.UserId);

	//std::string showSessionlocation = m_realPModel->getParam(RPARAM_SHOWSESSIONWORKSTATION).c_str();
	if ( m_realPModel->isShowSessionLocation() )
	{
		...
		text+= "@";
		text+= m_realPModel->getWorkstationName(sessionInfoEx.WorkstationId);
	}
    ...
    if (m_realPModel->getCurrentSessionId() == sessionInfoEx.SessionId)
    {
...
{
    updateOperator(row, m_realPModel->getSessionInfo(sessionId));
}
...
BEGIN_MESSAGE_MAP(RegionDutyPropertyPage, AbstractPropertyPage)
...
BOOL RegionDutyPropertyPage::OnInitDialog() 
{
    ...
    if (m_realPModel->isUseLocationLabel()) 
    {
        m_transferRegionDutyBtn.SetWindowText( TALoadString( STRING_ID_DUTY_MANAGER_0012 ).str() );
...
void RegionDutyPropertyPage::OnTransferRegionDutyBtn() 
{
    try
	{
		if ( TA_Base_App::DutyManagerDlg::m_isMartixDataInited == true )
		{
			...
			RegionDutyTransferDlg rgnDutyXferDlg( this->m_pWinApp );
		...
		else
		{
			...
			MessageBox::info(TALoadString( STRING_ID_DUTY_MANAGER_0013 ).toMultiByteStdString() );
		}
    }
    catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
    {
		std::string exStr(ex.what());
		std::string exDetailStr("");
		size_t found;
		found=exStr.find("TRANSIENT_CallTimedout");
		if(found == std::string::npos)
		{
			MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0041).toMultiByteStdString(), exStr,exDetailStr);
		}
		else
		{
			MessageBox::warning(TALoadString(STRING_ID_DUTY_MANAGER_0042).toMultiByteStdString());
		}
    }
    catch (const TA_Base_Core::TransactiveException& ex)
    {
		std::string exStr(ex.what());
		std::string exDetailStr("");
        MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0043).toMultiByteStdString(), exStr,exDetailStr);
    }
    catch (...)
    {
        MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0043).toMultiByteStdString());
    }
}
...
void RegionDutyPropertyPage::OnDelegateSubDutyBtn() 
{
	...
	CString errMsg = userMsg.constructMessage(IDS_UW_220001).str();
	// TD14164 ++azenitha
	...
	if ( TA_Base_App::DutyManagerDlg::m_isMartixDataInited == true )
	{
		...
		if (MessageBox(errMsg,
			TALoadString( STRING_ID_DUTY_MANAGER_0006 ).str(),
			MB_ICONWARNING | MB_YESNO | MB_DEFBUTTON2) == IDYES)
			// TD14164 ++azenitha
		{
			try
			{
				...
				m_realPModel->degradeAllSubsystems();
			}
			catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
			{
				LOG_EXCEPTION_CATCH(SourceInfo, "DutyMonitorFrameworkException", ex.what());
				...
				MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0039).toMultiByteStdString(), exStr,exDetailStr);
			}
		}
	}
	else
...
void RegionDutyPropertyPage::OnSessionUpdate(const SessionUpdate& sessionUpdate)
{
    ...
    try
    {
        ...
        if (sessionUpdate.sessionId == m_realPModel->getCurrentSessionId())
        {
            enableDutyControlButtons();
        }
    }
...
void RegionDutyPropertyPage::OnRegionDutyUpdate(const RegionDutyUpdate& dutyUpdate)
{
    ...
    for (RegionDutyList::const_iterator rgnDutyIt = dutyUpdate.duties.begin(); rgnDutyIt != dutyUpdate.duties.end(); rgnDutyIt++)
    {
        try
        {
            SessionInfoEx sessionInfoEx(m_realPModel->getSessionCashe().getSessionInfo(rgnDutyIt->session.in()));
