...
OtherSesSubDutyPropertyPage::OtherSesSubDutyPropertyPage(ITransActiveWinApp * pWinApp, UINT nIDTemplate, UINT nIDCaption  ) :
TA_Base_Bus::AbstractPropertyPage(pWinApp, OtherSesSubDutyPropertyPage::IDD, nIDCaption),
m_realPModel(NULL),
...
std::string OtherSesSubDutyPropertyPage::getMyPModelName()
{
	 return PMODEL_OTHER_SES_SUB_DUTY_PROPERTY_PAGE;
}

void OtherSesSubDutyPropertyPage::setupPModel()
{
	m_realPModel = dynamic_cast<OtherSesSubDutyPropertyPageModel*> (m_pModel);
	TA_ASSERT(m_realPModel != NULL, "PModel should be valid");
	
}

void OtherSesSubDutyPropertyPage::initUIControl()
{
		enableButtonsAndSessionInfoEdit(false);
}

TA_Base_Bus::DataCache& OtherSesSubDutyPropertyPage::getDataCache()
{
	return m_realPModel->getDataCache();
}

TA_Base_Bus::SessionCache& OtherSesSubDutyPropertyPage::getSessionCashe()
{
	return m_realPModel->getSessionCashe();
}


TA_Base_Bus::SubsystemDutyMatrix& OtherSesSubDutyPropertyPage::getSubsystemDutyMatrix()
{
	return m_realPModel->getSubsystemDutyMatrix();
}

std::string OtherSesSubDutyPropertyPage::getRegionDisplayName(RegionKey key)
{
	return m_realPModel->getRegionDisplayName(key);
	// m_dataCache.getRegionDisplayName(*regionIt).c_str();
}

std::string OtherSesSubDutyPropertyPage::getRegionName(RegionKey key)
{
	return m_realPModel->getRegionName(key);
	// m_dataCache.getRegionName(*regionIt).c_str();
}

void OtherSesSubDutyPropertyPage::notifyUpdate(const char * updateType, TA_Base_Bus::EventArgs * args)
{

}
void OtherSesSubDutyPropertyPage::requestUserReply(TA_Base_Bus::UIReplyEventArgsPtr uiEvent)
{
...
void OtherSesSubDutyPropertyPage::initialiseOhterSessionDutyGrid()
{
    ...
    SubsystemKeyList subsystemKeys(m_realPModel->getSubsystemDutyMatrix().getRowIdsByRights(m_realPModel->getCurrentSessionInfo().ProfileId));
    RegionKeyList regionKeys(m_realPModel->getSubsystemDutyMatrix().getColumnIds());
...
void OtherSesSubDutyPropertyPage::OnShowAllSubsCheckBtn() 
{
    ...
    SubsystemKeyList allSubsystemKeys(m_realPModel->getSubsystemDutyMatrix().getRowIds());
    SubsystemKeyList accessibleSubsystemKeys(m_realPModel->getSubsystemDutyMatrix().getRowIdsByRights(m_selectSessionInfo.ProfileId));
...
void OtherSesSubDutyPropertyPage::OnSelectSessionBtn() 
{
	...
	try
    {
		...
		{
			m_selectSesSubDutyDlg = new SelectSesSubDutyDlg(m_pWinApp);
		}

		LOG_GENERIC(SourceInfo,DebugUtil::DebugDebug,"m_selectSesSubDutyDlg->DoModal()");
	...
	TA_Base_Bus::RegionKeyList regionKeys) //TD15533
{
    ...
    catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
    {
        ...
        MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0036).toMultiByteStdString(), ex.what(),"");
    }
	...
	const TA_Base_Bus::ESubsystemState subsystemState)
{
	...
	try
	{
		...
		else if (profilesWithDuties.size() == 0)
		{
			if (m_selectSessionInfo.ProfileId.size() == 2)
			{
				TA_Base_Bus::ActionGroupKey baseActionGroupKey = 
					m_realPModel->getActionGroupDB(m_selectSessionInfo.ProfileId[0], subsystemKey, subsystemState);
				TA_Base_Bus::ActionGroupKey overrideActionGroupKey = 
					m_realPModel->getActionGroupDB(m_selectSessionInfo.ProfileId[1], subsystemKey, subsystemState);
		...
		{
			TA_Base_Bus::SessionInfoEx sessionInfoEx = getSessionCashe().getSessionInfo( m_selectSessionInfo.SessionId );
...
void OtherSesSubDutyPropertyPage::updateOtherSesSubDrid(TA_Base_Bus::SessionInfoEx sesInfEx)
{
	...
	{
		subsystemKeys = getSubsystemDutyMatrix().getRowIds();
	}
	else
	{
		subsystemKeys = getSubsystemDutyMatrix().getRowIdsByRights(m_selectSessionInfo.ProfileId);
	}
    ...
    for (RegionKeyList::const_iterator regionIt = regionKeys.begin(); regionIt != regionKeys.end(); regionIt++)
    {
        ...
        try
        {
            ...
            gridItem.row = REGION_ROW;
            gridItem.col = column;
            gridItem.szText = getDataCache().getRegionDisplayName(*regionIt).c_str();
            gridItem.lParam = *regionIt;

			regionName = getDataCache().getRegionName(*regionIt).c_str();

			CString allLocations(ALL_LOCATOINS.c_str());
			if(0 == regionName.CompareNoCase(allLocations))
			{
				m_sesSubDutyGrid.DeleteColumn(column);
				continue;
			}
        ...
        catch (const TA_Base_Bus::DutyMonitorFrameworkException& ex)
        {
            ...
            MessageBox::error(TALoadString(STRING_ID_DUTY_MANAGER_0037).toMultiByteStdString(), ex.what(),"");
        }
    }
	...
	if(retryTime > CHECK_SESSION_IN_DB_RETRY_TIMES)
	{
		...
		//CString errMsg = TALoadString( STRING_ID_DUTY_MANAGER_0000 ).c_str();//"During initialise Subsystem DutyGrid, no session persisted in DB";
		//CString errMsg2 = TALoadString( STRING_ID_DUTY_MANAGER_0001 ).c_str();//"Still failed after retry"
		if (MessageBox(MsgBoxErrCStr,
                   MsgBoxErrCStr2,
                   MB_ICONWARNING | MB_OK | MB_DEFBUTTON2) == IDOK)
		{
...
void OtherSesSubDutyPropertyPage::updateOtherSessionInfoEdit(TA_Base_Bus::SessionInfoEx sesInfEx)
{
	...
	bool isGet =m_realPModel->getSessionInfoName(sesInfEx,operatorName,profileName,locationName,workstationName);
	if (false == isGet)
	{
	...
	//operatorName = getDataCache().getOperatorName(sesInfEx.OperatorId[0]);
	//profileName = getDataCache().getProfileName(sesInfEx.ProfileId[0]);
	...
	//	operatorName += getDataCache().getOperatorName(sesInfEx.OperatorId[1]);
	//				
	//	profileName += "/";
	//	profileName += getDataCache().getProfileName(sesInfEx.ProfileId[1]);
	//}
	//locationName = getDataCache().getLocationDisplayName(sesInfEx.LocationId);
	//workstationName = m_realPModel->getWorkstationName(sesInfEx.WorkstationId);

	//CString csText;
	//csText.Format(_T("Selected Session:  Operator: %s,    Profile: %s,    Location: %s,    Workstation: %s,    SessionId: %s"),operatorName.c_str(),profileName.c_str(),locationName.c_str(),workstationName.c_str(),sesInfEx.SessionId.c_str());
	TAUnicodeString tempStr = TAAfxFormatStringX(STRING_ID_DUTY_MANAGER_0038,
		CONVERT_FROM_STD(operatorName),
		CONVERT_FROM_STD(profileName), 
		CONVERT_FROM_STD(locationName),
		CONVERT_FROM_STD(workstationName),
		CONVERT_FROM_STD(sesInfEx.SessionId),NULL,NULL,NULL,NULL		);

	
	m_selectedSessionEdit.SetWindowText(tempStr.c_str());
	UpdateData(FALSE);
	enableButtonsAndSessionInfoEdit(true);
...
void OtherSesSubDutyPropertyPage::OnSessionUpdate(const SessionUpdate& sessionUpdate)
{
	{
...
void OtherSesSubDutyPropertyPage::sessionUpdateRegionDuty(const SessionUpdate& sessionUpdate)
{
	//
}

void OtherSesSubDutyPropertyPage::sessionUpdateSubsystemDuty(const SessionUpdate& sessionUpdate)
{
    ...
    m_selectSessionInfo = getSessionCashe().getSessionInfo(selectSessionId);

	SubsystemKeyList previousSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(previousProfileId));
    SubsystemKeyList currentSubsystemKeys(getSubsystemDutyMatrix().getRowIdsByRights(m_selectSessionInfo.ProfileId));
	...
	if (m_showAllSubsystemsOtherSesBtn.GetCheck() == BST_UNCHECKED)
	{
		...
		updateSubsystems(empty, currentSubsystemKeys, m_realPModel->getCurrentSessionInfo().ProfileId);
	}
	else
	{
		updateSubsystems(allSubsystemKeys, empty, m_realPModel->getCurrentSessionInfo().ProfileId);
        updateSubsystems(empty, allSubsystemKeys, m_realPModel->getCurrentSessionInfo().ProfileId);
    }
}
...
void OtherSesSubDutyPropertyPage::OnSubsystemDutyUpdate(const SubsystemDutyUpdate& dutyUpdate)
{
    ...
    if ( ( TA_Base_Bus::DUT_ADD == dutyUpdate.updateType ) || ( TA_Base_Bus::DUT_REMOVE == dutyUpdate.updateType ) )
    {
        for (SubsystemDutyList::const_iterator subDutyIt = dutyUpdate.duties.begin(); subDutyIt != dutyUpdate.duties.end(); subDutyIt++)
        {
            try
            {
				...
				CString baseText(cellData.baseText.c_str());
				CString overrideText(cellData.overrideText.c_str());
				m_sesSubDutyGrid.SetItemImage(row, column, cellData.baseImage);
				m_sesSubDutyGrid.SetItemText(row, column, baseText);
                m_sesSubDutyGrid.SetItemImageS(row, column, cellData.overrideImage);
				m_sesSubDutyGrid.SetItemTextS(row, column, overrideText); 
				...
				else*/ if ((cellData.baseText.compare("") == 0) && (cellData.overrideText.compare("") == 0))
				{
					...
					m_sesSubDutyGrid.SetItemText(row, column, tempStr);
					m_sesSubDutyGrid.SetItemImageS(row, column, -1);
					m_sesSubDutyGrid.SetItemTextS(row, column, tempStr); 
