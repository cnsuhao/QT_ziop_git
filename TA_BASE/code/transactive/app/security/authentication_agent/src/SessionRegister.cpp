 ...
 * 
 * Source:    $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/security/authentication_agent/src/SessionRegister.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #2 $
 *
        ...
        if( isControl )
        {
            ...
            for( it=ownedSessions.begin(); it!=ownedSessions.end(); it++ )
            {
				...
				if ( !(*it)->getHasEnded() )
				{
					...
					m_sessionActiveTimes.insert( std::make_pair<std::string, time_t>( sessionInfo.SessionId.c_str(), time(0) ) );
				}
				// ++ly
			...
			else
			{
				...
				if ( getOwnedSessionInfoFromDb(sessionId, tmpSession) )
				{
					...
					*/	
					...
					{
						TA_Base_Core::ThreadGuard guardB( m_activeTimesAccessLock );
						m_sessionActiveTimes.insert(std::make_pair<std::string, time_t>(tmpSession.SessionId.c_str(), time(0)));
					}
