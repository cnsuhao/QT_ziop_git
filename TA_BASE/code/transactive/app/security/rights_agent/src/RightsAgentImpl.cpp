 ...
 * 
 * Source:    $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/security/rights_agent/src/RightsAgentImpl.cpp $
 * @author:   Nick Jardine
 * @version:  $Revision: #2 $
 *
...
namespace TA_Base_App
{
            ...
            TA_Base_Bus::IRightsAgentCorbaDef::EDecisionModule& decisionModule)
    {
		...
		while ( (!resultKnown) && ( plIt != profileList.end() ) )
		{
			...
			{
			case TA_Base_Bus::PdpConst::PERMIT_PR:{
				if(isPermittedOnResource == true)
				{atLeastOnePermit = true;
				resultKnown = true;}
				else atLeastOneDeny = true;
					break;
												  }
				case TA_Base_Bus::PdpConst::DENY_PR:
					atLeastOneDeny = true;
