...
int CSrvComWorker::_SocketWriteMsgPackage(IMsgPackage* pSendMsgPackage)
{
	...
	{
		m_nHeartBeatState = enHeartBeat_Timer_OFF;
		LOG1(SourceInfo, DebugUtil::DebugMsg, "set HeartBeatState OFF [%s]", m_strClientID.c_str());		
...
{	  	
		...
		MSGPACKAGE_PT_REQUEST == pEachMsgPkg->m_nPkgType)
	{
		...
		LOG1(SourceInfo, DebugUtil::DebugMsg, "set HeartBeatState ON [%s]", m_strClientID.c_str());
