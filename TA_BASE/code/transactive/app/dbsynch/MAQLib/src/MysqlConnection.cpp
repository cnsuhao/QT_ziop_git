...
{	
	...
	if (NULL == m_pConnection)
	{
		...
		TA_ASSERT(NULL != g_p_db_driver, "Mysql database Driver is null");
		while (!bIsOk /*&& i < uSrvIpSize*/)
		{
			try
			{
				...
				m_pConnection = g_p_db_driver->connect(connection_properties); 
				m_pConnection->setSchema(strDbName);

				// set this session mode with backslash options based on the configuration
				if (g_p_db_con_opt && CON_BACKSLASH_OFF == g_p_db_con_opt->m_nConSlashSwitch)
				{
...
sql::Driver*  g_p_db_driver  = 0;
MAQ_CON_OPT*  g_p_db_con_opt = 0;

void  g_attach_db_driver(PMAQ_CON_OPT pConOpt)
{
	if (!g_p_db_driver)
	{
		g_p_db_driver = (sql::Driver*)get_driver_instance();
		//g_p_db_driver->threadInit();
	}
	if (!g_p_db_con_opt && pConOpt)
	{
		g_p_db_con_opt = new MAQ_CON_OPT();
		g_p_db_con_opt->m_nConSlashSwitch = pConOpt->m_nConSlashSwitch;
	}

}

void  g_deattach_db_driver()
{
	/*if (g_p_db_driver)
	{
		g_p_db_driver->threadEnd();
	}*/
	DEF_DELETE(g_p_db_con_opt);
}
