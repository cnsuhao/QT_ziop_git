/**
  ...
  * in any form.
  *
  * Source: $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/data_management/archive_manager/src/BasicHelper.cpp $
...
// 		}
    ...
    {            
            ...
            ( '_' !=         fileName[11] ) )
        {
			...
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050110_INVALID_FILENAME_FORMAT,fileName);
    ...
    std::string getOperatorNameFromSessionId()
    {
        ...
        try
        {
            ...
            {
				std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050111_OPERATOR_NAME,UE050111_REASON_OP_FAILED);
                TA_THROW( TA_Base_Core::ArchiveException(  error ) );
			}
        ...
        catch( TA_Base_Core::DatabaseException& e )
        {
			...
			std::string message = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050111_OPERATOR_NAME, e.what());
        ...
        bool isManualArchive )
    {
        ...
        try
        {
            ...
            if ( 0 == data.get() )
            {
				std::string message = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050113_AUDIT_MSG,UE050113_REASON_ENT_FAILED);
                TA_THROW( TA_Base_Core::ArchiveException(message) );
            }
        ...
        {
            LOG_EXCEPTION_CATCH( SourceInfo, "DatabaseException", e.what() );
			std::string message = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050113_AUDIT_MSG,e.what());
            TA_THROW( TA_Base_Core::ArchiveException(message) );
        }
        catch( TA_Base_Core::DataException& e )
        {
        ...
        {
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResource(UE050114_MSG_SENDER);
            TA_THROW( TA_Base_Core::ArchiveException(  error ) );
        }
        ...
        catch( TA_Base_Core::TransactiveException& e )
        {
			...
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050115_AUDIT_MSG_FAILED,e.what());
            TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
        catch( ... )
        {
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050115_AUDIT_MSG_FAILED,UE050115_REASON_UNKNOWN);
            TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
    }
    ...
    bool isActionPermitted( unsigned long actionKey )
    {
        ...
        try
        {
            ...
            if ( 0 == data.get() )
            {
				std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050087_UNKNOWN_RIGHTS,UE050087_REASON_UNKNOWN_RESOURCE);
                TA_THROW( TA_Base_Core::ArchiveException( error ) );
            }
        ...
        {
            LOG_EXCEPTION_CATCH( SourceInfo, "DatabaseException", e.what() );
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050087_UNKNOWN_RIGHTS, e.what());
            TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
        catch( TA_Base_Core::DataException& e )
        {
        ...
        if ( 0 == library )
        {
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050087_UNKNOWN_RIGHTS, UE050087_REASON_RL_ACCESS_FAILURE);
            TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
        ...
        catch( ... )
        {
			...
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050087_UNKNOWN_RIGHTS,"Caught an unknown exception during rights checking.");
            TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
    ...
    TA_ArchiveManager::EErrorStatus processFile( const std::string& file, bool isImporting )
    {
       ...
       typedef std::map< std::string, TA_ArchiveManager::EErrorStatus > TokenMap;
        ...
        {
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050116_CHECK_LOG_FAILED,file,UE050116_REASON_INVALID_FILE);
			TA_THROW( TA_Base_Core::ArchiveException( error ) );
       }
        ...
        if ( !in.is_open() )
        {
			std::string error = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UE050116_CHECK_LOG_FAILED,file,UE050116_REASON_OPEN_FAILED);
			TA_THROW( TA_Base_Core::ArchiveException( error ) );
        }
    ...
    void deleteLogFiles( const TA_ArchiveManager::FileStatusMap& files )
    {
        ...
        if( !error.str().empty() )
        {
			std::string errorStr = TA_Base_App::IGuiObserver::getInstance()->getStringResourceFormated(UW050015_DELETE_LOG_FILES_FAILED,error.str().c_str());
