/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/data_management/archive_manager/src/DeleteArchiveDlg.cpp $
  * @author:  Alan Brims
  * @version: $Revision: #1 $
  *
...
namespace
{
    ...
    const TCHAR* DELETED				( _T("Deleted")   );
    const TCHAR* DELETING			( _T("Deleting")  );
    const TCHAR* AVAILABLE			( _T("Available") );
    const TCHAR* DELETE_FAILED		( _T( "Failed")   );
    const TCHAR* CANCELLED			( _T("Cancelled") );
}

namespace TA_Base_App
{
    DeleteArchiveDlg::DeleteArchiveDlg(CWnd* pParent /*=NULL*/)
        : AbstractDialog(dynamic_cast<TA_Base_Bus::ITransActiveWinApp*> (AfxGetApp()),DeleteArchiveDlg::IDD, pParent),
          m_status( IDOK )
    {
    ...
    BEGIN_MESSAGE_MAP(DeleteArchiveDlg, AbstractDialog)
	...
	LRESULT DeleteArchiveDlg::OnForceCancel(WPARAM wp, LPARAM lp)
	{
		...
		message = TALoadString( UI050050_DELETE_IN_PROG );
		//-- td19009 bgn
		...
		for ( int item = 0; item < m_archiveList.GetItemCount(); item++ )
		{
			...
			if( 1 == state )
			{
				if( m_archiveList.GetItemText( item, STATUS_COL ) == DELETED )
				{
					m_archiveList.SetItemState( item, INDEXTOSTATEIMAGEMASK(0), LVIS_STATEIMAGEMASK );
				}
				else
				{
					m_archiveList.SetItemText( item, STATUS_COL, CANCELLED);
				}
			}
		}
		//-- td19009 end
    ...
    void DeleteArchiveDlg::run()
    {
        ...
        try
        {
	          ...
	          for ( int item = 0; item < itemCount; item++ )
            {
                ...
                if( 1 == state )
                {
                    m_archiveList.SetItemText( item, STATUS_COL, DELETING );
                    try
                    {
                        // KT 27 Feb 04: PW #3133: This has been changed so that users can remove
                        // an individual days worth of restored data from the database.  The 
                        ...
                        {
							TAUnicodeString date(_T("selected date"));
                            TAUnicodeString reason;
                            reason = TALoadString( UE050054_REASON_FIND_FAILED );
                            TAUnicodeString error;
                            error.str().FormatMessage( UE050054_REMOVE_FAILED,date, reason );
                            
                            TA_Base_Bus::TransActiveMessage userMsg;
                            userMsg << date << reason;
                            UINT selectedButton = userMsg.showMsgBox(IDS_UE_050054);

                            m_archiveList.SetItemText( item, STATUS_COL, DELETE_FAILED );
                        }

                        m_deleteArchiveHelper.deleteRestoredData( key, it->second.dataDate );
                        m_archiveList.SetItemText( item, STATUS_COL, DELETED );
                    }
                    catch( TA_Base_Core::ArchiveException& e )
                    {
                        LOG_EXCEPTION_CATCH( SourceInfo, "ArchiveException", e.what() );
    ...
    BOOL DeleteArchiveDlg::OnInitDialog() 
    {
        ...
        int statusWidth =  pDC->GetTextExtent( CANCELLED ).cx + 15;
        //TD16728++
        //Hanhao
        ...
        m_archiveList.InsertColumn( DATA_DATE_COL,  TALoadString(IDS_DATA_DATE_COL_STR).str(),        LVCFMT_LEFT, dateWidth, -1);
        m_archiveList.InsertColumn( RESTORED_BY_COL,      TALoadString(IDS_RESTORED_BY_COL_STR).str(),      LVCFMT_LEFT, whoWidth,  -1);
        m_archiveList.InsertColumn( RESTORATION_DATE_COL, TALoadString(IDS_RESTORED_DATE_COL_STR).str(), LVCFMT_LEFT, dateTimeWidth, -1);
        m_archiveList.InsertColumn( DELETION_DUE_DATE_COL,TALoadString(IDS_DEL_DUE_DATE_COL_STR).str(),    LVCFMT_LEFT, deletionWidth, -1);
        m_archiveList.InsertColumn( STATUS_COL,           TALoadString(IDS_STATUS_COL_STR).str(),           LVCFMT_LEFT, statusWidth, -1);
    ...
    void DeleteArchiveDlg::populateList()
    {
        ...
        TAUnicodeString dataDate( _T("") );
        TAUnicodeString restorationDate( _T("") );
        TAUnicodeString deletionDueDate(_T("") );
        int itemIndex = 0;
        ...
        {   
			...
			dataDate = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::convertTimeTToStringDate( it->second.dataDate ).c_str());
			restorationDate = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::convertTimeTToStringDate( it->second.restoredDate, true ).c_str());
			deletionDueDate = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::convertTimeTToStringDate( it->second.deletionDueDate ).c_str());

            itemIndex = m_archiveList.InsertItem( m_archiveList.GetItemCount(), NULL );
            m_archiveList.SetItemText(itemIndex, DATA_DATE_COL, dataDate.str() );
            m_archiveList.SetItemText(itemIndex, RESTORED_BY_COL, TAUnicodeString::CreateFromMultitByteString(it->second.restoredBy.c_str()).str());
            m_archiveList.SetItemText(itemIndex, RESTORATION_DATE_COL, restorationDate.str() );
            m_archiveList.SetItemText(itemIndex, DELETION_DUE_DATE_COL, deletionDueDate.str() );
            m_archiveList.SetItemText(itemIndex, STATUS_COL, AVAILABLE);  // We only show available items, not data that is being deleted.
    ...
    void DeleteArchiveDlg::OnButtonRemoveData() 
    {
		...
		for ( int item = 0; item < m_archiveList.GetItemCount(); item++ )
		{
			...
			if( 1 == state )
			{
				m_archiveList.SetItemText(item, STATUS_COL, AVAILABLE);
			}
		}
		start();
    }
    ...
    void DeleteArchiveDlg::OnModifyButton() 
    {
        ...
        if ( IDCANCEL == status )
        {
            ...
            try
            {
				TAUnicodeString operatorName = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::getOperatorNameFromSessionId().c_str());
				TAUnicodeString workstationName = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::getWorkstationNameFromSessionId().c_str());

                TA_Base_Bus::TransActiveMessage userMsg;
                userMsg << operatorName << workstationName;
                UINT selectedButton = userMsg.showMsgBox(IDS_UI_050057);

                message.str().FormatMessage( UI050057_CHANGE_DATE_ABORTED, operatorName.c_str(), workstationName.c_str());
            }
            catch( TA_Base_Core::ArchiveException& e )
            {
                LOG_EXCEPTION_CATCH( SourceInfo, "ArchiveException", e.what() );
                message = TALoadString( UI050060_TRANSFER_ABORTED );

                TA_Base_Bus::TransActiveMessage userMsg;
                UINT selectedButton = userMsg.showMsgBox(IDS_UI_050060);
            }
            
			helper.completeChangeDeletionDate( IDCANCEL, message.toMultiByteStdString());
            setEnabledProperty( true );
            return;
        }
        else if ( IDOK == status )
        {
            ...
            catch( TA_Base_Core::ArchiveException& e )
            {
                LOG_EXCEPTION_CATCH( SourceInfo, "ArchiveException", e.what() );
                TAUnicodeString error;
                error.str().FormatMessage( UE050117_SELECT_FAILED, e.what() );
                
                TA_Base_Bus::TransActiveMessage userMsg;
                userMsg << e.what();
                UINT selectedButton = userMsg.showMsgBox(IDS_UE_050117);

				helper.completeChangeDeletionDate( IDABORT, error.toMultiByteStdString() );
                setEnabledProperty( true );
                return;
            }
            ...
            {
                TAUnicodeString reason;
                reason=TALoadString( UE050118_REASON_INV_ACCESS_FAILED );
                TAUnicodeString error;
                error.str().FormatMessage( UE050118_UPDATE_FAILED, reason );
	          ...
	          for ( int item = 0; item < m_archiveList.GetItemCount(); item++ )
            {
                ...
                if( 1 == state )
                {
                    ...
                    try
                    {
						...
						TAUnicodeString deletionDueDateStr = TAUnicodeString::CreateFromMultitByteString(TA_ArchiveManager::convertTimeTToStringDate( newDeletionDate ).c_str());
						m_archiveList.SetItemText( item, DELETION_DUE_DATE_COL, deletionDueDateStr.str());
                    }
                    catch( TA_Base_Core::DatabaseException& e )
                    {
                        LOG_EXCEPTION_CATCH( SourceInfo, "DatabaseException", e.what() );
                        TAUnicodeString error;
                        error.str().FormatMessage( UE050118_UPDATE_FAILED, e.what() );
	...
	std::string DeleteArchiveDlg::getMyPModelName()
	{
		return ARCHIVE_MANAGER_DLG_MODEL;
	}

	void DeleteArchiveDlg::setupPModel()
	{

	}

	void DeleteArchiveDlg::notifyUpdate(const char * updateType, TA_Base_Bus::EventArgs * args)
	{
	}
