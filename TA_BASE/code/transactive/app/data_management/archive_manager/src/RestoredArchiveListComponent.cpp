/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/data_management/archive_manager/src/RestoredArchiveListComponent.cpp $
  * @author:  Wenguang
  * @version: $Revision: #1 $
  *
...
//19726++
	...
	#ifdef _DEBUG
				headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(_T("ArchiveManager.exe")),
...
{  
	...
	TAUnicodeString   strItem1,   strItem2;  
	LVFINDINFO   info;  
	int   nIndex;  
	...
	int iCompRes = strItem1.str().Compare(strItem2.str());
	if (0 == iCompRes)
	{
		return 0;
	}
	...
	if(1 == pV->m_nCurrentSortColumn
		...
		// ++ CL-16719
	{  
		iCompRes = SortDate(strItem1.toMultiByteStdString(), strItem2.toMultiByteStdString());
	}  
	else 
	{
		iCompRes = strItem1.str().Compare(strItem2.str());  
	}
...
BOOL RestoredArchiveListComponent::OnToolTipText(UINT id, NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    *pResult = 0;
    ...
    if(pNMHDR->code == TTN_NEEDTEXTA)
    {
        ...
        lstrcpyn( m_pchTip.get(), strTip.str(), strTip.GetLength() );
    ...
    else
    {
		...
		_mbstowcsz( m_pwchTip.get(), strTip.toMultiByteStdString().c_str(), strTip.GetLength() ); 
