/**
  ...
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_agent/src/DssEngine.cpp $
  * @author:  Huang Qi
  * @version: $Revision: #1 $
  *
...
const char* const DEACTIVATE_PLAN_REASON = "Stopped according to DSS rule";
...
void DssEngine::processTriggerUpdate()
{
    ...
    if ( bNeedDeactivate )
    {
        for ( std::list<TA_Base_Core::ActivePlanId>::iterator itLoop = lstActivePlans.begin(); lstActivePlans.end() != itLoop; ++itLoop )
        {
            ...
            try
            {
                PlanExecutor::instance()->stop( *itLoop, DEACTIVATE_PLAN_REASON );
            }
