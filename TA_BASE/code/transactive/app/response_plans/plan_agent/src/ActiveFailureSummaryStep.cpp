/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_agent/src/ActiveFailureSummaryStep.cpp $
	...
	TA_Base_Core::EActiveStepState ActiveFailureSummaryStep::executeImpl( void )
	{
		...
		try
		{
            ...
            if (NULL != executionHistory)
            {
                ...
                {
                    std::string remark = ActivePlanBulletinBoard::instance()->getActivePlanDetail( m_flowControl.getPlan() ).remark.in();
