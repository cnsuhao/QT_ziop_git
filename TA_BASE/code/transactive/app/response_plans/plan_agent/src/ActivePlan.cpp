/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_agent/src/ActivePlan.cpp $
...
namespace TA_Base_App
{
	...
	void ActivePlan::pause( void )
	{
        ...
        if (!m_isCurrentInteractiveStep)
        {
			...
			if (m_remark.empty ())
			{
   		   ...
   		   ActivePlanBulletinBoard::instance()->update( m_plan, getStepNumber(), TA_Base_Core::PAUSED_ACTIVE_STEP_STATE, m_remark );
        }
	...
	void ActivePlan::setNextStep( const unsigned long step )
	{
	...
	void ActivePlan::setNextStep( void )
	{
		...
		ActivePlanBulletinBoard::instance()->update( m_plan, TA_Base_Core::COMPLETED_ACTIVE_PLAN_STATE, m_remark );
	...
	bool ActivePlan::interlockOk( void )
	{
		...
		switch( result )
		{
			...
			case SUCCESS_WR:
			{
				...
				ActivePlanBulletinBoard::instance()->update( m_plan, m_lastExecutedStep, TA_Base_Core::EXECUTED_ACTIVE_STEP_STATE, m_remark );
				rval = true;
			}
			break;
			...
			case FAILURE_WR:
			{
				...
				ActivePlanBulletinBoard::instance()->update( m_plan, m_lastExecutedStep, TA_Base_Core::FAILED_ACTIVE_STEP_STATE, m_remark );
				ActivePlanBulletinBoard::instance()->update( m_plan, TA_Base_Core::FAILED_ACTIVE_PLAN_STATE, m_remark );
				setIsActive( false );
				rval = false;
			}
			break;
	...
	void ActivePlan::customiseStepImpl( const TA_Base_Core::StepCustomisableDetail& newDetail )
	{
		...
		if( m_steps.end() != it )
		{
			...
			ActivePlanBulletinBoard::instance()->update(m_plan, newDetail );	
	...
	{
		ActivePlanBulletinBoard::instance()->update( m_plan, sessionId, pmId );
