/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_agent/src/PlanExecutionEngine.cpp $
...
// 		    m_activationQueue.enqueue( new RunPlanMethodObject( *this, activePlan ) );
	...
	void PlanExecutionEngine::runImpl( std::auto_ptr<ActivePlan> activePlan )
	{
		...
		while( activePlan->getIsActive() )
		{
            ...
            {
                LOG_EXCEPTION_CATCH( SourceInfo, "ActivePlanMessageException", expWhat.what() );
                break;
            }
            catch (...)
            {
	...
	void PlanExecutionEngine::processMessages( ActivePlan& activePlan, boost::shared_ptr<ActivePlanMessageQueue> mq )
    {
		...
		else // we have a message
        {
            ...
            catch( const std::bad_cast& )
            {
                ...
                std::ostringstream what;
                what << "[P:" << activePlan.getPlan().plan << ",I:" << activePlan.getPlan().instance << "]:" << " Dynamic cast to ActivePlanMessageBlock failed.";
			...
			try
			{
				...
				ACE_Read_Guard<ACE_RW_Mutex> guard(m_mqMapLock);
				{
					...
					else
					{
						...
						std::ostringstream what;
						what << "[P:" << apMb->getPlan().plan << ",I:" << apMb->getPlan().instance << "]:" << " Message for unknown plan discarded.";
