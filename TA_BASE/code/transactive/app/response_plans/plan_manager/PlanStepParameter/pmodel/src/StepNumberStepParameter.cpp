/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/StepNumberStepParameter.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
void StepNumberStepParameter::addToParameterList(CPlanStepParameterListCtrlModel &listCtrl)
{
...
std::string StepNumberStepParameter::getDestinationStepDescription()
{
    ...
    if(destinationStep != NULL)
    {
        ...
        stepDescription = boost::lexical_cast<std::string>(stepNumber);
        if(stepNumber > 0)
        {
            stepDescription += " - ";
