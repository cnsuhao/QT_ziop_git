...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterCameraEditDlg.cpp $
    ...
    for(it = cameras.begin(); it != cameras.end(); ++it)
    {
        ...
        if(!groupName.empty())
        {
            ...
            else
            {
                ...
                HTREEITEM rootItem = m_cameraTree.InsertItem(SCOPEDMTOW(groupName.c_str()), 0, 0, TVI_ROOT, TVI_LAST);
                if(NULL != rootItem)
                {
        ...
        if(0 == groupRootItem)
        {
            cameraItem = m_cameraTree.InsertItem(SCOPEDMTOW(cameraName.c_str()), 2, 2, TVI_ROOT, TVI_LAST);
        }
        else
        {
            cameraItem = m_cameraTree.InsertItem(SCOPEDMTOW(cameraName.c_str()), 2 , 2, *groupRootItem, TVI_LAST);
        }
...
void CPlanStepParameterCameraEditDlg::onOK()
{
    ...
    m_cameraParameter.setCameraName(SCOPEDWTOM(LPCTSTR(m_cameraName)));
