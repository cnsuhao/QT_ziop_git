...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterMonitorEditDlg.cpp $
...
void CPlanStepParameterMonitorEditDlg::loadMonitorTree()
{
    ...
    for(it = groups.begin(); it != groups.end(); it++)
    {
        ...
        rootItem = m_monitorTree.InsertItem(SCOPEDMTOW(groupName.c_str()), 0, 0, TVI_ROOT, TVI_LAST);
        if(NULL == rootItem)
        {
        ...
        for(unsigned int i = 0; i < videoOutputs->length(); i++)
        {
            ...
            monitorItem = m_monitorTree.InsertItem(SCOPEDMTOW(monitorName.c_str()), 2 , 2, rootItem, TVI_LAST);
            if(NULL == monitorItem)
            {
...
void CPlanStepParameterMonitorEditDlg::onOK()
{
    ...
    m_monitorParameter.setMonitorName(SCOPEDWTOM(LPCTSTR(m_monitorName)));
