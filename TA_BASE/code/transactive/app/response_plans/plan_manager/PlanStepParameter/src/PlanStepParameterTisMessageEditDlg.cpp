/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterTisMessageEditDlg.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
CPlanStepParameterTisMessageEditDlg::CPlanStepParameterTisMessageEditDlg(unsigned short libraryVersion, TisMessageStepParameter& parameter, bool canEdit) :
    ...
    m_parameterName(SCOPEDMTOW(parameter.getName().c_str())),
    m_libraryType(SCOPEDMTOW(Utilities::convertToString(parameter.getMessageType()).c_str())),
    ...
    m_messageLibraryVersion(_T(""))
{
    ...
    {
        m_messageLibraryVersion = TA_Base_Bus::StringUtilities::convertToString(message.libraryVersion).c_str();
    }
...
BOOL CPlanStepParameterTisMessageEditDlg::OnInitDialog()
{
    ...
    m_messageList.InsertColumn(TYPE_COLUMN_INDEX, LabelMaker::getTisMessageListTypeColumnLabel(), LVCFMT_LEFT, 90);
    m_messageList.InsertColumn(ID_COLUMN_INDEX, LabelMaker::getTisMessageListIdColumnLabel(), LVCFMT_LEFT, 40);
    m_messageList.InsertColumn(MESSAGE_COLUMN_INDEX, LabelMaker::getTisMessageListMessageColumnLabel(), LVCFMT_LEFT, 410);
    m_messageList.InsertColumn(PRIORITY_COLUMN_INDEX, LabelMaker::getTisMessageListPriorityColumnLabel(), LVCFMT_LEFT, 60);
            ...
            mIt != m_predefinedMessages.end(); mIt++)
    {
        ...
        m_messageList.SetItemText(item, TYPE_COLUMN_INDEX, SCOPEDMTOW(Utilities::convertToString(librarySection).c_str()));
        m_messageList.SetItemText(item, ID_COLUMN_INDEX, TA_Base_Bus::StringUtilities::convertToString(mIt->messageTag).c_str());
        m_messageList.SetItemText(item, MESSAGE_COLUMN_INDEX, SCOPEDMTOW(mIt->message.c_str()));
        m_messageList.SetItemText(item, PRIORITY_COLUMN_INDEX, TA_Base_Bus::StringUtilities::convertToString(mIt->priority).c_str());
