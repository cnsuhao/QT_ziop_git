/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterTisPidListEditDlg.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
#include "app/response_plans/plan_manager/src/PlanStepParameterTisPidListEditDlg.h"
#include "TisPidListStepParameter.h"
#include "app/response_plans/plan_manager/src/TisConfigAccess.h"
...
BOOL CPlanStepParameterTisPidListEditDlg::OnInitDialog()
{
    ...
    for(unsigned long i(0); i < selectedPids.length(); i++)
    {
        ...
        {
            m_selectedPidList.AddString(SCOPEDMTOW(pidIt->c_str())); // valid PID, ok to add to Selected PIDs box
    ...
    {
        m_availablePidList.AddString(SCOPEDMTOW(pidIt->c_str()));
    }
...
void CPlanStepParameterTisPidListEditDlg::OnOK()
{
    ...
    for(int item(0); item < m_selectedPidList.GetCount(); item++)
    {
        ...
        m_selectedPids.push_back(SCOPEDWTOM(LPCTSTR(pid)));
    }
