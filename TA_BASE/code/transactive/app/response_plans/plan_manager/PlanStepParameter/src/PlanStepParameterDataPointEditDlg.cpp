...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepParameterDataPointEditDlg.cpp $
// @author:  Andy Parker
// @version: $Revision: #1 $
//
...
void CPlanStepParameterDataPointEditDlg::OnOK()
{
    ...
    UpdateData();
    parameter->setDataPoint(SCOPEDWTOM(LPCTSTR(m_dataPointName)));
...
void CPlanStepParameterDataPointEditDlg::OnScadaTreeItemSelectionChanged(NMHDR* pNMHDR, LRESULT* pResult)
{
    ...
    if(selectedItem)
    {
        ...
        m_dataPointName = (isDataPoint ? (SCOPEDMTOW(selectedItem->getEntityData()->getName().c_str())) : _T(""));
