/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanNode.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #3 $
  *
...
#include "app/response_plans/plan_manager/pmodel/src/PlanAgentAccessException.h"
...
const std::string &PlanNode::getNodeType()
{
	...
	static const std::string PLAN_NODE_TYPE = LabelMaker::getPlanNodeTypeLabel();
...
const std::string &PlanNode::getNodeStatus()
{
	...
	static const std::string APPROVED_PLAN_STATUS = "Commented";//Utilities::convertToString(TA_Base_Core::APPROVED_APPROVAL_STATE);
	static const std::string UNAPPROVED_PLAN_STATUS = "Commented";//Utilities::convertToString(TA_Base_Core::UNAPPROVED_APPROVAL_STATE);
...
const std::string PlanNode::getNodeImageType(bool isExpanded)
{
	...
	FUNCTION_EXIT;
	return (isApproved() ? ICON_APPROVED_PLAN : ICON_UNAPPROVED_PLAN);
}
...
//void PlanNode::updatePlanView(PlanGeneralDetailTab &generalDetailPage, const PlanInstanceFilter &filter)
//{
...
PlanInstanceList* PlanNode::getAllLocationPlans(const PlanInstanceFilter &filter)
{
...
//        TA_Base_Bus::TA_Rect newRect = TA_Base_Bus::AppLauncher::getInstance().getRect(
