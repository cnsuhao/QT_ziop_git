...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanInstanceFilter.cpp $
// @author:  Bart Golab
// @version: $Revision: #1 $
//
...
bool PlanInstanceFilter::isRelevant(const TA_Base_Core::ActivePlanId& activePlanId)
{
    ...
    if (planNode)
    {
        PlanInstance *planInstance = planNode->getPlanInstance(PlanInstanceId(activePlanId.instance, activePlanId.location));
        if (planInstance)
        {
            FUNCTION_EXIT;
...
bool PlanInstanceFilter::isRelevant(const PlanInstance& planInstance)
{
    FUNCTION_ENTRY( "isRelevant" );
