...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanControllerDoc.cpp $
// @author:  Bart Golab
// @version: $Revision: #1 $
//
...
BEGIN_MESSAGE_MAP(CPlanControllerDoc, CPlanBrowserDoc)
    ...
    ON_COMMAND(ID_PLAN_RESUME_SELECTED_STEP, OnResumeSelectedPlan)
            ...
            case ID_PLAN_RESUME_SELECTED_STEP:
                if (GetCurrentInstance())
                {
                    GetCurrentInstance()->updateCmdUI(*pCmdUI, *planNode,
                        (GetCurrentStep() == NULL) ? 0 : GetCurrentStep()->getStepPosition());
                    break;
                }
            case ID_PLAN_STOP:
            case ID_PLAN_PAUSE:
...
void CPlanControllerDoc::OnStopPlan()
{
    ...
    try
    {
        PlanInstance *planInstance = GetCurrentInstance();
...
void CPlanControllerDoc::OnResumeSelectedPlan()
{
    FUNCTION_ENTRY( "OnResumeSelectedPlan" );
    ...
    try
    {
        ...
        if (planInstance != NULL)
        {
            ...
            TA_ASSERT(planStep, "No plan step selected");

            if (planStep != NULL)
            {
                planInstance->resumAtStep(planStep->getStepPosition());
            }
        }
    }
    catch (TA_Base_Core::TransactiveException &ex)
