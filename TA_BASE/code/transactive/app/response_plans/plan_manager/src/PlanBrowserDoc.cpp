...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanBrowserDoc.cpp $
// @author:  Bart Golab
// @version: $Revision: #2 $
//
...
//TODO: move to self messagemapping
...
CPlanBrowserDoc::CPlanBrowserDoc(TA_Base_Bus::IApplicationController* appController, const char * pmodelName) :
...
void CPlanBrowserDoc::OnActivePlanDetailUpdate()
{
    ...
    while (!m_activePlanDetailUpdateQueue.empty())
    {
        ...
        TA_Base_Core::ActivePlanDetail* activePlanDetailUpdate = m_activePlanDetailUpdateQueue.front();
        m_activePlanDetailUpdateQueue.pop_front();
...
void CPlanBrowserDoc::OnPlanConfigDataUpdate()
{
    ...
    while (!m_planConfigDataUpdateQueue.empty())
    {
        TA_Base_Core::PlanConfigUpdateData* planConfigDataUpdate = m_planConfigDataUpdateQueue.front();
        ...
        if (planConfigDataUpdate)
        {
            ...
            TA_Base_Core::PlanConfigUpdateData& planConfigData = *planConfigDataUpdate;
...
void CPlanBrowserDoc::OnPlanCustomisationUpdate()
{
    ...
    while (!m_planCustomisationUpdateQueue.empty())
    {
        ...
        TA_Base_Core::PlanCustomisationUpdate* planCustomisationUpdate = m_planCustomisationUpdateQueue.front();
        m_planCustomisationUpdateQueue.pop_front();
...
void CPlanBrowserDoc::SetCurrentInstance(PlanInstance *instance)
{
...
PlanInstance *CPlanBrowserDoc::GetCurrentInstance() const
{
...
void CPlanBrowserDoc::activeStateChanged(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
    ...
    //{
        m_activePlanDetailUpdateQueue.push_back(new TA_Base_Core::ActivePlanDetail(activePlanDetail));
...
void CPlanBrowserDoc::planConfigChanged(const TA_Base_Core::PlanConfigUpdateData &planConfigData)
{
    ...
    //if (m_parentFrameHWND)
    //{
        m_planConfigDataUpdateQueue.push_back(new TA_Base_Core::PlanConfigUpdateData(planConfigData));
...
void CPlanBrowserDoc::planTreeNodeDeleted(const TA_Base_Core::PlanConfigUpdateData &planConfigData)
{
    ...
    //{
        ...
        // than PostMessage().
        TA_Base_Core::PlanConfigUpdateData* planConfigDataUpdate = new TA_Base_Core::PlanConfigUpdateData(planConfigData);
...
void CPlanBrowserDoc::planCustomised(const TA_Base_Core::PlanCustomisationUpdate &planCustomDetail)
{
    ...
    //if (m_parentFrameHWND)
    //{
        m_planCustomisationUpdateQueue.push_back(new TA_Base_Core::PlanCustomisationUpdate(planCustomDetail));
...
void TA_Base_App::CPlanBrowserDoc::processEvent( boost::shared_ptr<std::string> newEvent )
{
    if (0 == strcmp(newEvent->c_str(), MSG_ID_ACTIVE_PLAN_DETAIL_UPDATE))
    {
        OnActivePlanDetailUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_PLAN_CONFIG_DATA_UPDATE))
    {
        OnPlanConfigDataUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_PLAN_CUSTOMISATION_UPDATE))
    {
        OnPlanCustomisationUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_RPARAM_EXECPLANID_UPDATE))
    {
        OnRparamExecPlanIdUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_RPARAM_VIEWPLANID_UPDATE))
    {
        OnRparamViewPlanIdUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_RPARAM_ASSOC_ALARM_UPDATE))
    {
        OnRparamAssocAlarmIdUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_RPARAM_LOCKCATEGORY_UPDATE))
    {
        OnRparamLockCategoryUpdate();
    }
    else if (0 == strcmp(newEvent->c_str(), MSG_ID_RPARAM_SESSIONID_UPDATE))
    {
        OnRparamSessionIdUpdate();
    }
