...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanStepDetailPage.cpp $
// @author:  Bart Golab
// @version: $Revision: #1 $
//
...
BOOL CPlanStepDetailPage::OnSetActive()
{
    ...
    {
        PlanInstance *currentInstance = GetDocument()->GetCurrentInstance();
...
void CPlanStepDetailPage::OnPlanInstanceComboItemSelected()
{
    ...
    PlanInstance *planInstance = m_planInstanceCombo.GetSelectedInstance();
    bool isActiveInstance = (dynamic_cast<PlanInstanceNone *>(planInstance) == NULL);

    GetDocument()->SetCurrentInstance(isActiveInstance? planInstance : NULL);
    ...
    catch (const TA_Base_Core::TransactiveException& ex)
    {
        ...
        if (isActiveInstance)
        {
            m_planInstanceCombo.SetSelectedInstance(NULL);
        }
    }
...
LRESULT CPlanStepDetailPage::OnSelectActivePlanInstance(WPARAM wParam, LPARAM lParam)
{
    ...
    PlanInstance *planInstance = GetPlanNode()->getPlanInstance(planInstanceId);
...
LRESULT CPlanStepDetailPage::OnActivePlanDetailUpdate(WPARAM wParam, LPARAM lParam)
{
    ...
    if (planInstance)
    {
        if (m_planInstanceCombo.IsNewInstance(planInstance))
        {
            m_planInstanceCombo.AddInstance(planInstance);
        ...
        {
            if (planInstance == m_planInstanceCombo.GetSelectedInstance())
            {
...
LRESULT CPlanStepDetailPage::OnPlanCustomisationUpdate(WPARAM wParam, LPARAM lParam)
{
    ...
    if (planInstance && planInstance == m_planInstanceCombo.GetSelectedInstance())
    {
...
ICellManager::CellType CPlanStepDetailPage::GetCellType(CEditableListCtrl* listCtrl, int item, int subItem)
{
    ...
    PlanInstance *planInstance = GetDocument()->GetCurrentInstance();
...
TAUnicodeString CPlanStepDetailPage::GetCellMask(CEditableListCtrl* listCtrl, int item, int subItem)
{
...
bool CPlanStepDetailPage::UpdateCellValue(CEditableListCtrl* listCtrl, int item, int subItem,  const TAUnicodeString& value)
{
