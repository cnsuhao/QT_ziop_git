...
// in any form.
//
// Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanInstance.cpp $
// @author:  Bart Golab
// @version: $Revision: #1 $
//
...
TA_Base_Core::ActivePlanDetail& TA_Base_App::PlanInstance::getActivePlanDetail()
{
    FUNCTION_ENTRY( "getActivePlanState" );
...
void PlanInstance::updateCmdUI(CCmdUI &cmdUI, PlanNode &planNode, PlanStepNumber stepPosition)
{
    ...
    switch (cmdUI.m_nID)
    {
        ...
        case ID_PLAN_RESUME_SELECTED_STEP:
            {
                StepExecDetailMap& stepExecMap = getStepExecDetailMap();
                 if (stepExecMap.find(stepPosition))
                {
                    TA_Base_Core::EActiveStepState stepState = stepExecMap[stepPosition].state;
                    bool isCurrentStepRunning = (stepState == TA_Base_Core::EXECUTED_ACTIVE_STEP_STATE);
                    bool isCurrentStepPaused = (stepState == TA_Base_Core::PAUSED_ACTIVE_STEP_STATE);
                    bool isCurrentStepSkipped = (stepState == TA_Base_Core::SKIPPED_ACTIVE_STEP_STATE);
                    cmdUI.Enable(m_ownedByCurrentSession && isPaused  &&
                        OperatorRights::getInstance().canControlPlans() &&
                        (isCurrentStepPaused));
...
void PlanInstance::addToPlanInstanceCombo(CPlanInstanceComboBox &instanceComboBox)
{
    ...
    instanceComboBox.AddInstance(this);
...
//lizettejl++ (DP-changes: resume plan instance at the specified step position)
void PlanInstance::resumAtStep(PlanStepNumber stepPosition)
{
    FUNCTION_ENTRY( "resumAtStep" );

    PlanAgentAccess::getInstance().resumePlanAtStep( getActivePlanId() , stepPosition);
