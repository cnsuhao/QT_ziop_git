/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/response_plans/plan_manager/src/PlanAgentUpdateProcessor.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
void PlanAgentUpdateProcessor::processActivePlanDetailUpdate(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
    ...
    #define TRACE_ACTIVE_PLAN_UPDATES
#ifdef TRACE_ACTIVE_PLAN_UPDATES
    ...
    switch (activePlanDetail.currentStepDetail.state)
    {
          ...
          case TA_Base_Core::UNDEFINED_ACTIVE_STEP_STATE:
             stepState = "undefined";
             break;
    }

    LOG(SourceInfo, TA_Base_Core::DebugUtil::GenericLog, TA_Base_Core::DebugUtil::DebugDebug,
        "Update [%lu] - Plan [%lu:%s] - Location [%s] - Instance [%lu] - "
        "Owner [%s] - Step [%lu:%s] - Plan State [%s] - Step State [%s]",
        activePlanDetail.activeUpdateId, activePlanDetail.plan.plan, activePlanDetail.path.in(),
        TA_Base_Bus::PlanAgentLocationAccess::getInstance().getLocationName(activePlanDetail.plan.location).c_str(),
        activePlanDetail.plan.instance, activePlanDetail.owner.in(), activePlanDetail.currentStepDetail.position,
        activePlanDetail.currentStepDetail.name.in(), planState.c_str(), stepState.c_str());

#endif // TRACE_ACTIVE_PLAN_UPDATES
