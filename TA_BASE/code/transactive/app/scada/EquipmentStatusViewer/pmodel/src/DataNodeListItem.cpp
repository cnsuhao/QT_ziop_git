/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/scada/EquipmentStatusViewer/src/DataNodeListItem.cpp $
  * @author:  Alan Brims
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    DataNodeListItem::DataNodeListItem(boost::shared_ptr<TA_Base_Core::DataNodeEntityData> entityData,
										...
										EquipmentItemPopulateLogic* populateLogic)
        : EquipmentStatusListItem(entityData->getKey(), populateLogic),
		  m_dataNode(NULL)
    {
        //  This may throw a ScadaProxyException so be prepared to catch it.
        //  If we can't createDataNodeProxy() then try reallocating this DataNodeListItem later.
    ...
    std::string DataNodeListItem::buildNewColumnText( TA_Base_App::EslColumns column )
    {
    ...
    std::string DataNodeListItem::buildSummaryText() 
    {
		int numSummaries = 0;
    ...
    bool DataNodeListItem::checkFilters(FilterFlagsAndStrings const& filters)
    {
	...
	bool DataNodeListItem::updateTextIfChanged( EslColumns column, std::string& newText)
	{
	...
	{
		m_text[ASSET_COLUMN] = (*m_dataNode)->getAssetName();
		m_text[DESCRIPTION_COLUMN] = (*m_dataNode)->getLabel();		
	}
