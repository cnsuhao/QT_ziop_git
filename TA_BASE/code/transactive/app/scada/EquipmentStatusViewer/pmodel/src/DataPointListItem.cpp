/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/scada/EquipmentStatusViewer/src/DataPointListItem.cpp $
  * @author:  Alan Brims
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    DataPointListItem::DataPointListItem(unsigned long entitykey, TA_Base_Bus::ScadaProxyFactory& proxyFactory, EquipmentItemPopulateLogic* populateLogic)
        : EquipmentStatusListItem(entitykey, populateLogic),
		  m_dataPoint(NULL),
		  m_dataNode(NULL)
	{
	...
	DataPointListItem::DataPointListItem(boost::shared_ptr<TA_Base_Core::DataPointEntityData> dpEntityData, 
		TA_Base_Bus::ScadaProxyFactory& proxyFactory, EquipmentItemPopulateLogic* populateLogic)
		: EquipmentStatusListItem(dpEntityData->getKey(), populateLogic),
		  m_dataPoint(NULL),
		  m_dataNode(NULL)
	{
    ...
    std::string DataPointListItem::buildStatusText()
    {
		int numInhibits = 0;
    ...
    std::string DataPointListItem::buildNewColumnText( TA_Base_App::EslColumns column )
    {
        ...
        try
        {
            ...
            case VALUE_COLUMN:
                {
                    ...
                    newText = (*m_dataPoint)->getValueAsString();
                    break;
                }
            case QUALITY_COLUMN:
                {
            ...
            case MODIFIED_BY_COLUMN:
                {
                    if (!getColumnText(STATUS_COLUMN).empty())
                    {
            ...
            /*case TIMESTAMP_COLUMN:
                {
                    if (!getColumnText(STATUS_COLUMN).IsEmpty() )
                    {
                        ...
                        if (0L != m_timeStamp)
                        {
		                    ...
		                    std::string formatStr = dateFormat.getDateFormat(TA_Base_Core::DateFormat::DateTimeFormat4).c_str(); 
    ...
    bool DataPointListItem::checkFilters( FilterFlagsAndStrings const& filters)
    {
        if ( filters[REPORT_TYPE_FILTER_OFFSET].filterInUse )
        {
        ...
        return EquipmentStatusListItem::checkFilters(filters);
    }
	...
	bool DataPointListItem::updateTextIfChanged( EslColumns column, std::string& newText)
	{
	...
	void DataPointListItem::initStaticColumn()
	{
		if(m_dataNodeEntity.get() != NULL)
		{
			m_text[ASSET_COLUMN] = m_dataNodeEntity->getAssetName();
			m_text[DESCRIPTION_COLUMN] = m_dataNodeEntity->getLabel();
		}

		m_text[DATAPOINT_COLUMN] = (*m_dataPoint)->getDescription();
		m_text[TYPE_COLUMN] = (*m_dataPoint)->getShortNameOfType();
	}
} // TA_Base_App
