/**
  ...
  * any form.
  *
  * Source:		$File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/scada/OpcBridgeLib/src/OpcTagManager.cpp $
...
namespace TA_Base_App
{
    ...
    std::string OpcTagManager::getTagLocation ( const std::string & tagName ) const
	{
		...
		//
		int pos = tagName.find_first_of( ".", 0 );
		if ( pos > 0 )
		{
    ...
    std::string OpcTagManager::getTagObjectName ( const std::string & tagName ) const
	{
		...
		int pos = tagName.find_last_of ( ".", tagName.length() );
