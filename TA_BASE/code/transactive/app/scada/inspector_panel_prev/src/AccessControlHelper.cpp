/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/scada/inspector_panel_prev/src/AccessControlHelper.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
	...
	{
		// always clear the whole map if not cleared
		m_entityAccessControlMap.clear();
		...
		for (std::vector<AccessEntityInfo>::iterator iter = entityInfo.begin();
												iter != entityInfo.end();
												iter ++)
		{			
			// find out if each of actions is allowed on this entity
			std::map<int, int>::iterator actionIterator;
			...
			m_entityAccessControlMap.insert(
