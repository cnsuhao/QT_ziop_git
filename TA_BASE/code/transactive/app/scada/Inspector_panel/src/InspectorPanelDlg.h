/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/scada/inspector_panel/src/InspectorPanelDlg.h $
  * @author:  Andy Parker
  * @version: $Revision: #1 $
  *
...
{
	class CInspectorPanelDlg : public TA_Base_Bus::AbstractTransActiveDialog//, public TA_Base_Core::RunParamUser
	{			
	// Construction
	public:

		CInspectorPanelDlg(TA_Base_Bus::ITransActiveWinApp * app);	// standard constructor
		virtual ~CInspectorPanelDlg();

		std::string		getMyPModelName(void);
		void			setupPModel(void);
		//void			notifyPModelDataChange( const char * updateType, const TA_Base_Bus::EventArgs * args/*=NULL*/ );
		...
		CString	getInitStatusString(EquipmentInitStatus status);
...
//        afx_msg LRESULT OnProcessRunParamInMainThread(UINT wParam, LONG lParam);
		...
		afx_msg LRESULT OnResizeWindow(UINT wParam, LONG lParam);
...
//		LRESULT OnLoadDataPoints (WPARAM wParam, LPARAM lParam);
	...
	protected:
		...
		ETabNumbers								m_previousTab;
		ETabNumbers								m_requestedTab;
		...
		bool									m_isInitialTab;
		InspectorPanelDlgModel*					m_realPModel;
