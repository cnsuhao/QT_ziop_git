/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/scada/inspector_panel/src/DataPointControlHelper.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
				...
				if (valueStream >> doubleValue)
     			{
						...
						|| doubleValue < dataPoint->getLimit(true))
					{
						...
						std::string* statusLineStr = new std::string(message.str());
//todo 						AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);
// 						//CInspectorPanelDlg::addStatusLine ( message.str().c_str() );
// 
...
//todo 					std::string* statusLineStr = new std::string(message.str());
// 					AfxGetApp()->m_pMainWnd->PostMessage(UPDATE_STATUS_WINDOW_MESSAGE, reinterpret_cast<UINT>(statusLineStr), 0);
					//CInspectorPanelDlg::addStatusLine ( message.str().c_str() );
