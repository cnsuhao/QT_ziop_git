/**
  ...
  *
  * Source:    $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/scheduling/scheduling_agent/src/SchedulingMonitor.cpp $
...
void SchedulingMonitor::receiveJobUpdatedMessage(const TA_Base_Bus::SchedulingAgentCorbaDef::JobCorbaDef& updatedJob)
{
    ...
    // Used if an alarm is raised - the action that is being performed
    std::ostringstream action;
    action << "update job with ID '" << updatedJob.identifier.in() << "'" 
        << " and description '" << updatedJob.description.in() << "'";
