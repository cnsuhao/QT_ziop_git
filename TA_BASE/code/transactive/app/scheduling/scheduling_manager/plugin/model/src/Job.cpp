/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/scheduling/scheduling_manager/plugin/model/src/Job.cpp $
  * @author:  San Teo
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    m_startDate(NULL)
    {
        ...
        unsigned int index = jobInfo.find(SchedulingManager::DELIMITER_JOBTYPE, 0);
        ...
        while ((index != std::string::npos) && (index < jobInfo.size()))
        {
            ...
            unsigned int valueIndex = jobInfo.find(SchedulingManager::DELIMITER_JOBPARAMVALUE, index);
            if (valueIndex == std::string::npos)
            {
                // if there's parameter name but no value, then don't set it, just return
                return;
            }
            unsigned int nextIndex = jobInfo.find(SchedulingManager::DELIMITER_JOBPARAMNAME, valueIndex);
