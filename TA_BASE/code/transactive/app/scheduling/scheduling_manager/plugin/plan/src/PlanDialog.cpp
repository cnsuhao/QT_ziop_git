/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/scheduling/scheduling_manager/plugin/plan/src/PlanDialog.cpp $
  * @author:  San Teo
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_App
{
    ...
    PlanDialog::PlanDialog(bool readOnly, const TAUnicodeString& selectedPlanPath, CWnd* pParent /*=NULL*/)
    ...
    BOOL PlanDialog::OnInitDialog() 
    {
        ...
        if (!m_selectedPlanPath.IsEmpty())
        {
            try
            {
				...
				std::string strPlanPath = m_selectedPlanPath.toMultiByteStdString();
                m_planCtrl.setSelectedPlan(strPlanPath);
            }
            catch (...)
            {
    ...
    void PlanDialog::OnPlanTreeItemSelected(NMHDR* pNMHDR, LRESULT* pResult) 
    {
        ...
        {
            m_selectedPlanPath = TAUnicodeString::CreateFromMultitByteString(planItem->getPlanPath().c_str());
