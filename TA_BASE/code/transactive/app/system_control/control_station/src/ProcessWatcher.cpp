/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/control_station/src/ProcessWatcher.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
        ...
        try
        {
            ...
            if (::OpenProcessToken(::GetCurrentProcess(), TOKEN_ADJUST_PRIVILEGES | TOKEN_QUERY, &hToken))
            {
                if (::LookupPrivilegeValue(NULL, SE_DEBUG_NAME, &privNameValue))
                {
                    ...
                    {
                        LOG(DEBUG_INFO, "Could not set privilege some option's may not wo");
                    }
                }
                ...
                if (::LookupPrivilegeValue(NULL, SE_SHUTDOWN_NAME, &privNameValue))
                {
                    ...
                    if (!::AdjustTokenPrivileges(hToken, false, &privileges, sizeof(privileges), NULL, &dwRet) ||
                            ::GetLastError() == ERROR_NOT_ALL_ASSIGNED)
                    {
                        LOG(DEBUG_INFO, "Could not set privilege some option's may not work");
                    }
                }
            }
        }
    ...
    try
    {
        while (!isTerminating())
        {
            ...
            for (; (iter != iterEnd) && (!isTerminating()); ++iter)
            {
                ...
                {
                    LOG(DEBUG_INFO, "[id=%d][name=%s] terminated", iter->first, iter->second.c_str());
