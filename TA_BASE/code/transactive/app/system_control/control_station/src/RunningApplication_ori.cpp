/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/control_station/src/RunningApplication.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
{
    //boost::function<void(unsigned long)> f = makeAsyncFunction(boost::bind(&RunningApplication::terminating, this, _1));
    //Signals::processTerminating[0].connect(boost::bind(&RunningApplication::terminating, this, _1));
    //Signals::processTerminating[0].connect(boost::bind(&RunningApplication::terminating, this, _1));
    //Signals::processTerminating[0].connect(f);
}
                                ...
                                const RECT& boundaryDim)
{
    ...
    LOG(DEBUG_INFO, "About to launch application: %s \nWith command line arguments: %s", executable.c_str(), arglist.c_str());
...
{
    FUNCTION_ENTRY("relaunch");

    LOG(DEBUG_DEBUG, "RunningApplication::relaunch - shouldTerminateOnLogout=%d, isRelaunchAvailable=%d, entityKey=%d, commandline=%s",
...
void RunningApplication::terminate()
{
    ...
    LOG(DEBUG_INFO, "Terminating application - %s", applicationName.c_str());

    // Check if the application is running. If it has already been stopped then there is no need to terminate again.
    if (!m_isRunning)
    {
        LOG(DEBUG_INFO, "Application has already been terminated");
        return;
    }
    ...
    try
    {
        ...
        {
            LOG(DEBUG_INFO, "Terminating as a Managed Process");
    ...
    {
        LOG(DEBUG_INFO, "Terminating as a normal process");
...
{
    FUNCTION_ENTRY("changeApplicationFocus");
...
void RunningApplication::serverStateChanged(bool isServerUp)
{
    FUNCTION_ENTRY("serverStateChanged");

    try
    {
        LOG(DEBUG_INFO, "Going to change the status of %s", getApplicationName().c_str());
...
void RunningApplication::passNewParameters(const std::string& additionalCommands)
{
    ...
    FUNCTION_ENTRY("passNewParameters");
    ...
    try
    {
        LOG(DEBUG_INFO, "Setting parameters in application: ");
...
void RunningApplication::passNewParameters(TA_Base_Core::RunParamSeq_var additionalParameters)
{
    ...
    try
    {
        ...
        // Now finally we can pass all these parameters on to the application
        LOG(DEBUG_INFO, "Repassing parameters to application: ");
...
{
    FUNCTION_ENTRY("setManagedProcess");
    m_managedProcess = managedProcess;
    FUNCTION_EXIT;
}
...
BOOL CALLBACK RunningApplication::focusNonManagedGui(HWND hWnd, LPARAM lParam)
{
    ...
    {
        LOG(DEBUG_DEBUG, "focusNonManagedGui::Found a window with matching ID");
        ...
        if (NULL != ::GetParent(hWnd))
        {
            ...
            LOG(DEBUG_DEBUG, "It's a subwindow. Ignore.");
            return TRUE;
        }
...
void RunningApplication::updateSessionId(std::string& sessionId)
{
    ...
    try
    {
        if (!CORBA::is_nil(m_managedProcess))
        {
            ...
            m_managedProcess->setParam(parameter);
            LOG(DEBUG_INFO, "Successfully set Session ID for %s, thread ID = %d, process ID = %d",
...
void RunningApplication::dutyChanged()
{
    ...
    try
    {
        if (!CORBA::is_nil(m_managedGUI))
        {
            ...
            LOG(DEBUG_INFO, "Notified process %s, thread ID = %d, process ID = %d of duty changes",
                getApplicationFullName().c_str(),
                static_cast<int>(m_processInformation.dwThreadId),
                static_cast<int>(m_processInformation.dwProcessId));
        }
