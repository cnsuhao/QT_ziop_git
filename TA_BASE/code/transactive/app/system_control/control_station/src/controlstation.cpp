/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/control_station/src/ControlStation.cpp $
...
int ControlStationApp::ExitInstance()
{
    ...
    //TD18084
    LOG(DEBUG_DEBUG, "start to clean up Session Manager");
    SessionManager::cleanUp();
    LOG(DEBUG_DEBUG, "finish clean up Session Manager");
    ...
    {
        LOG(DEBUG_DEBUG, "Needs to restart the workstation");
        restartWorkstation();
    }
    else
    {
        LOG(DEBUG_DEBUG, "Exiting to windows");
        exitFromWindows();
    }
    ...
    if (m_mutex != NULL)
    {
        LOG(DEBUG_DEBUG, "release the mutex for controlstation");
        m_mutex->Unlock();
        delete m_mutex;
        m_mutex = NULL;
    }
...
void ControlStationApp::restartWorkstation()
{
    ...
    {
        LOG(DEBUG_INFO, "Successfully initiate system reboot first time round");
    ...
    if (ExitWindowsEx(EWX_REBOOT, NULL) != 0)
    {
        ...
        LOG(DEBUG_INFO, "Successfully initiated system reboot second time round.");
        LOG(SourceInfo, DebugUtil::FunctionExit, "restart");
        FUNCTION_EXIT;
        return;
    }

    LOG(DEBUG_INFO, "Failed to reboot nicely, forcing reboot now.");
...
void ControlStationApp::exitFromWindows()
{
    ...
    {
        LOG(DEBUG_INFO, "Logging out of windows");
    ...
    {
        LOG(DEBUG_INFO, "Not required to log out of windows as windows explorer is running");
    }
...
void ControlStationApp::archiveLogFiles(const std::string& debugFileName)
{
    ...
    m_logsThread = new ArchiveLogsThread(debugFileName);
    m_logsThread->start();
}
