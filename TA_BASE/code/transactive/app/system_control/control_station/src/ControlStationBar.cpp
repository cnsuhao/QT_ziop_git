/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/control_station/src/ControlStationBar.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
BOOL ControlStationBar::initDialog()
{
    LOG(SourceInfo, DebugUtil::FunctionEntry, "initDialog");
    ...
    LOG(SourceInfo, DebugUtil::FunctionExit, "initDialog");
...
void ControlStationBar::processUpdate(const TA_Base_Core::ConfigUpdateDetails& updateEvent)
{
    ...
    case TA_Base_Core::Update:
    {
        ...
        while (iter != end)
        {
            if ((*iter)->getKey() == key)
            {
                LOG(DEBUG_INFO, "scada display %d has been updated", key);
                (*iter)->invalidate();
                break;
            }
    ...
    case TA_Base_Core::Delete:
    {
        ...
        while (iter != end)
        {
            if ((*iter)->getKey() == key)
            {
                LOG(DEBUG_INFO, "Deleting scada display %d from the scada display list", key);
                delete (*iter);
                *iter = NULL;
    ...
    case TA_Base_Core::Create:
    {
        ...
        if (found)
        {
            ...
            LOG(DEBUG_INFO, "scada display %d has already been added no need to add again", key);
        }
        else
        {
            LOG(DEBUG_INFO, "Adding scada display %d", key);
...
void ControlStationBar::OnSelchangeFavouritesCombo()
{
    ...
    if (0 == index)
    {
        LOG(DEBUG_INFO, "Selected configuration from favourites list. Will now load favourites edit dialog.");
    ...
    {
        LOG(DEBUG_INFO, "Selected favourite with index %d. Will now attempt to load favourite.",
            index);
