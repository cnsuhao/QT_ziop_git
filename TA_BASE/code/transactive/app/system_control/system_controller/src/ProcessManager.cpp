/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/system_controller/src/ProcessManager.cpp $
    ...
    void ProcessManager::setLocalDatabseStatusOffline()
    {
        ...
        while (dbIt != m_moniteredDatabases.end())
        {
            if (dbIt->second != 0)
            {
                ...
                if (dbIt->first.find(m_centralDbName) == -1)
                {
...
//         }
    ...
    void ProcessManager::notifyProcessData(const TA_Base_Core::ProcessData& peerProcessData)
    {
        ...
        std::string entity = CORBA::string_dup(peerProcessData.ProcessId);
        std::string peerHost = CORBA::string_dup(peerProcessData.host);
...
//          }
        ...
        */
        ...
        while (m_isRunning)
        {
...

            {
                ...
                for (processIt = m_stoppingProcessIDToTimeCountMap.begin();
                        processIt != m_stoppingProcessIDToTimeCountMap.end();)
                {
    ...
    void ProcessManager::registerWithPeer(const std::string& host)
    {
        ...
        // "corbaloc::1.2@hostname:port/PeerProcessManager"
        std::ostringstream peerProcessManagerString;
        peerProcessManagerString << "corbaloc::1.2@" << host << ":"
        ...
        try
        {
            ...
            if (!CORBA::is_nil(obj))
            {
                ...
                TA_Base_Core::CorbaUtil::enableZiop<TA_Base_Core::IProcessSubjectCorbaDef>(peer);
                TA_Base_Core::ThreadGuard guard(m_peerLock);
                m_peers[host] = peer;
            }
        }
    ...
    void ProcessManager::stopProcess(const std::string& entityName)
    {
        ...
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugError,
                        "Attempting to stop process %s that is not managed by the System Controller",
                        entityName.c_str());
        }
    }
...
//                  }
                                                                   ...
                                                                   int locationKey)
    {
        ...
        int firstComma = 0;
        int secondComma = agentNotifyHosts.find_first_of(",");
