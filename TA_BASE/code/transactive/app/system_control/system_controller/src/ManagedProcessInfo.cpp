/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/system_controller/src/ManagedProcessInfo.cpp $
...
TA_Base_Core::NonReEntrantThreadLockable TA_Base_App::ManagedProcessInfo::m_lock;
TA_Base_Core::NonReEntrantThreadLockable TA_Base_App::ManagedProcessInfo::m_changeStateLock;
    ...
    TA_Base_Core::EOperationMode ManagedProcessInfo::getOperationMode()
    {
        ...
        try
        {
			...
			else if (currentStatus == TA_Base_Core::Startup)
			{
				...
				// ++yanrong
				if (m_managedAgent != TA_Base_Core::IManagedAgentCorbaDef::_nil())
				{
...
// 	}
    ...
    // isAPeer
    //
    bool ManagedProcessInfo::isAPeer(const std::string& hostname) const
    {
        TA_Base_Core::ThreadGuard guard(m_lock);
    ...
    std::vector<std::string> ManagedProcessInfo::buildCommandLine(std::string& startPath)
    {
        ...
        if( !databaseConnectionString.empty() )
        {
            ...
            if (entityLocationName.compare(m_locationName) != 0)
            {
                ...
                int loc = databaseConnectionString.rfind(entityLocationName);
    ...
    void ManagedProcessInfo::performStart()
    {
		...
		for (int i = 0; i < nLoops; i++)
		{
            ...
            if ( (isRunning() || getProcessStatus() == TA_Base_Core::Terminating) && !stateReportedSuccessfully )
            {
