...
void InteractiveLoop::initCommands()
{
    ...
    addCommand(&InteractiveLoop::setparamthis, GuiCommand, "setparamthis", "setparamthis <name> [value]", "Set a runparam (and its optional value) for this (ConsoleManager)");;
}
...
void InteractiveLoop::initHelpString()
{
    ...
    for (Group2CommandListMap::value_type& v : m_group2Commands)
    {
        strm << groupNames[v.first] << ":\n";

        for (CommandPtr command : v.second)
        {
