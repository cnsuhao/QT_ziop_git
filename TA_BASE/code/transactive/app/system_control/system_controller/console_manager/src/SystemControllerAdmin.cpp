...
TA_Base_Core::ProcessDataSeq* SystemControllerAdmin::getProcessData()
{
    ...
    if (RunParams::getInstance().isSet("TestZIOP"))
    {
        ...
        {
            for (const std::string& name : m_agentNames)
            {
                if (i < MAX_SIZE)
                {
...
{
    std::vector<std::string> dummyAgents =
    {
        "Alarm", "Authentication", "Duty", "NotificationService", "Rights", "SystemStatus", "DataNode", "Mms", "OnlinePrinting", "Scheduling", "Plan", "Pa", "VideoSwitch"
    };
    ...
    for (const std::string& name : dummyAgents)
    {
...
{
    for (Name2ManagedProcessMap::value_type& v : m_processes)
    {
                                                   ...
                                                   const char* applicationName)
{
    ...
    boost::thread t(&SystemControllerAdmin::waitProcessThreadFunc, this, p_processID, process);
}
