...
void ControlStationRootDialog::loginDetailsChanged()
{
	...
	LOG_GENERIC( SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "operatorId=%s, profile=%s, Location=%s  updateTime=%ld, displayOnly=%d", atsProfile->operatorId.c_str(),atsProfile->profileId.c_str(),atsProfile->loginLocation.c_str(),atsProfile->updateTime,atsProfile->displayOnly);
