/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/system_control/banner/src/ComponentLibraryAccessor.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #2 $
  *
...
namespace TA_Base_App
{
            ...
            int width, int height, TA_Base_Bus::BannerFrameworkDialog* parentDialog)
    {
        ...
        for (std::vector<TA_Base_Core::IEntityData*>::iterator iter = pages.begin(); iter != pages.end(); ++iter)
        {
            ...
            currentComponent.instance = new QLibrary(currentComponent.path.c_str());
			currentComponent.instance->setLoadHints(QLibrary::ResolveAllSymbolsHint | QLibrary::ExportExternalSymbolsHint);
            if ( false == currentComponent.instance->load() )
		    {
                LOG_GENERIC( SourceInfo, DebugUtil::DebugError, "Could not load the plugin %s", currentComponent.path.c_str() );
                unavailableComponents += currentComponent.path.c_str();
			...
			DLLCreateComponent = (pfnCreateComponent)currentComponent.instance->resolve("createComponent");//::GetProcAddress( currentComponent.instance, "createComponent" );
		    if ( NULL == DLLCreateComponent )
		    {
				...
				currentComponent.instance->unload();
				delete currentComponent.instance;
        ...
        if ( !unavailableComponents.isEmpty() )
        {
            ...
            userMsg << unavailableComponents.toStdString();
