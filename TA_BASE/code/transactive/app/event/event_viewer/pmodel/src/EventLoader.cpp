 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/event/event_viewer/EventListComponent/src/EventLoader.cpp $
 * @author:  Nick Jardine
 *
...
#include "app/event/event_viewer/pmodel/src/EventLoader.h"
...
{ 
	//TODO check and remove unwanted code....
	//changxing  TD14760
	...
	//CTimeSpan elapref;
	time_t startTime,endTime,endrefreshTime;
	time(&startTime);	
	double refsec=0;
	double elaps;
    ...
    while (!m_isTerminated)
    {
		...
		time(&endTime);
		elaps = difftime(endTime,startTime);
...
//         }
        ...
        {
			//changxing TD14760
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "elaps-%.f:refresh-%d",elaps,refreshsec);
			time(&startTime);
            refreshList();
			time(&endrefreshTime); 
			refsec = difftime(endrefreshTime, startTime);
			LOG_GENERIC(SourceInfo, DebugUtil::DebugInfo, "the refresh costs %.f seconds",refsec);
