...
namespace TA_Base_App{
	...
	unsigned long EventHandlerManager::s_refreshRate = 0; 
	...
	void EventHandlerManager::statusBarUpdateWhenStartQuery(){
		...
		if(!m_ISCSEnabled)
		{
			m_msg_ISCS = EVDisplayStrRes::GetInstance()->STATUS_ICSC_DISABLED;
		}else
		{
			m_msg_ISCS =  EVDisplayStrRes::GetInstance()->STATUS_ISCS_QUERYING;
		}

		if(!m_ATSEnabled)
		{
			m_msg_ATS =  EVDisplayStrRes::GetInstance()->STATUS_ATS_DISABLED;
		}
		else
		{
			m_msg_ATS =  EVDisplayStrRes::GetInstance()->STATUS_ATS_QUERYING;
		}
		m_msg= m_msg_ISCS + STATUS_SEPERATE + m_msg_ATS;
		sendStatusBarUpdate( 0, m_msg.c_str() );
	...
	unsigned long EventHandlerManager::getRefreshRate()
  	{
		return s_refreshRate;
	}

	void EventHandlerManager::setRefreshRate(const unsigned long refresh)
	{
		s_refreshRate = refresh;
	}
