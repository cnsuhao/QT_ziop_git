...
namespace TA_Base_App{
	...
	{
		IATSComWrapper::getInstanceHistorical()->doComInitialize(); 
		...
		IATSComWrapper::getInstanceHistorical()->cleanUp();
	}
	...
	void ATSHistoricalWorker::doCurrentTask()
	{
		...
		{
			actionCommand = IATSComWrapper::getInstanceHistorical()->doPreviousPageQuery(events, m_filter, EventHandlerManager::getMaxATSEvents()); 
			
		}
		else if( m_filter.searchDirection == TA_Base_App::Forward )
		{
			actionCommand = IATSComWrapper::getInstanceHistorical()->doNextPageQuery(events, m_filter, EventHandlerManager::getMaxATSEvents()); 
		}
		...
		if (!m_needToTerminate && !IsThisTaskStopped(m_curTaskSeqID))
		{
			...
			{		
				...
				m_wokerObserver->EventNotification(TA_Base_Core::ATSCategory, events, actionCommand,m_curTaskSeqID);
