...
namespace TA_Base_App{
	...
	void ATSRealTimeWorker::stopLastSubscriptionIfNeeded()
	{
		...
		if( m_ARunningSubscriptionExist){
			...
			actionCommand = IATSComWrapper::getInstanceRealTime()->stopATSSubscription(); //if stop unsuccessfully, will not try again.
	...
	void  ATSRealTimeWorker::startNewSubscription()
	{
			...
			m_filter.SubsystemNameEqualOr.size() == m_filter.appSubsystemIds.size() +  m_filter.physSubsystemIds.size() )
		{
			...
			m_wokerObserver->EventNotification(TA_Base_Core::ATSCategory, events, TA_Base_App::NoCommand,m_curTaskSeqID, true, true);
		...
		actionCommand =IATSComWrapper::getInstanceRealTime()->startATSSubscription(
			m_filter,
		...
		if(actionCommand == TA_Base_App::NoCommand)
		{
			...
			setStatus( FinishSearch);
			if( !m_needToTerminate && !m_wokerObserver->IsThisTaskStopped(m_curTaskSeqID) ) 
			{	
				m_wokerObserver->EventNotification(TA_Base_Core::ATSCategory, events, TA_Base_App::NoCommand,m_curTaskSeqID ,true, true);
			}
		...
		//subscribe failed: then subscribe again, but no need stopsubsciption because it's already passed.
			...
			m_wokerObserver->EventNotification(TA_Base_Core::ATSCategory, events, actionCommand,m_curTaskSeqID ,true);
			return;
		}
	...
	{
		IATSComWrapper::getInstanceRealTime()->doComInitialize(); //
		...
		while( !m_needToTerminate ){
			try
			{
				...
				else if( m_wokerObserver->getATSSubscriptionErr() < 0) // get ATSSubscriptionError, should restart the task for both ATS and ISCS
				{
					m_wokerObserver->setATSSubscriptionErr(0); // set it back to normal
					stopLastSubscriptionIfNeeded();
					m_wokerObserver->ATSSubscriptionErrNotification();
				}
				else  //should continue the current task
				{
					for (long i=0;i< (long)(EventHandlerManager::getRefreshRate()*5/1000) && !m_needToTerminate ;i++)
					{
		...
		m_hasTermainated = true;

		stopLastSubscriptionIfNeeded();

		//IATSComWrapper::freeInstanceRealTime();
		IATSComWrapper::getInstanceRealTime()->cleanUp();
	}
