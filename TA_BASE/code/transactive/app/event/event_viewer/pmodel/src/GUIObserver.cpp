...
#include "app/event/event_viewer/pmodel/src/GUIObserver.h"
...
namespace TA_Base_App 
{
	...
	{
		// Send the message.
		NotifyGUIPostMessage::getInstance().PostMessageToMainDlg(WM_USER_LIST_UPDATE, numElements, 0);
		NotifyGUIPostMessage::getInstance().PostMessageToMainDlg(WM_USER_LISTVIEW_UPDATE, numElements, 0);
	...
	{
		//Send the message.
		NotifyGUIPostMessage::getInstance().PostMessageToMainDlg( WM_USER_LOAD_COUNT, eventCount, reinterpret_cast<unsigned long>(text));
	...
	{
		// Send the message.
		NotifyGUIPostMessage::getInstance().PostMessageToMainDlg( WM_USER_LISTVIEW_UPDATE, numElements, pageStatus );
