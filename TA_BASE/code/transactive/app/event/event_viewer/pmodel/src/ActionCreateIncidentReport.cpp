/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/app/event/event_viewer/src/ActionCreateIncidentReport.cpp $
...
#include "app/event/event_viewer/pmodel/src/ActionIds.h"
...
void ActionCreateIncidentReport::executeAction( Filter* filter )
{
    ...
    if(selectedItems.size() == 1)
    {
        ...
        std::string alarmId( filter->getItemData( selectedItems[0], EVDisplayStrRes::GetInstance()->HIDDEN_COLUMN_ALARMID ) );

		//TD15870 xinsong++
       const std::string & eventValue = filter->getItemData( selectedItems[0], EVDisplayStrRes::GetInstance()->COLUMN_VALUE ) ;
		...
		std::string strAlarmValue = eventValue;
		size_t start_pos = 0;
		std::string from = " ";
		std::string to = "{%Blank%}";
		while((start_pos = strAlarmValue.find(from, start_pos)) != std::string::npos) {
			strAlarmValue.replace(start_pos, from.length(), to);
        ...
        commandLine << "  --alarm-value=" << strAlarmValue;
        ...
        catch(TA_Base_Core::ApplicationException&)
        {
			...
			NotifyGUIPostMessage::getInstance().showUserMsg(IDS_UE_040017);
        }
    }
