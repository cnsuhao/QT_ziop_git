...
namespace TA_Base_App{
	...
	void ISCSHistoricalWorker::load( FilterCriteria filterCriteria, long taskID )
	{
			...
			m_filter.appSubsystemIds.size() ==0 &&  m_filter.physSubsystemIds.size()==0 )
		{
			...
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Task(%d)ISCS Historical loading, get 0 events.",taskID);
			LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Task(%d)ISCS Historical loading stops.",taskID);
			return;
		}
		...
		try{
			...
			if(!m_needToTerminate && !IsThisTaskStopped(taskID)){
				...
				if( !m_needToTerminate && !IsThisTaskStopped(taskID) ){
					...
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Task(%d)ISCS Historical loading, get rest %d events.",taskID,restEvents.size());
					LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "Task(%d)ISCS Historical loading stops.",taskID);
