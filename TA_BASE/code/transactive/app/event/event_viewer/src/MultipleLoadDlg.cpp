/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/app/event/event_viewer/src/MultipleLoadDlg.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
MultipleLoadDlg::MultipleLoadDlg(const TAUnicodeString& windowTitle, CWnd* pParent /*=NULL*/)
...
void MultipleLoadDlg::OnMultiok() 
{
	...
	if(!IsAllItemSelected())
	{
		...
		for(unsigned int i = 0; i < selCount; i++)
		{
			...
			TAUnicodeString tmpStr;
			m_multiList.GetText(selIndex[i],tmpStr.str());
...
BOOL MultipleLoadDlg::OnInitDialog() 
{
	...
	if(m_includeAllItem)
	{
		m_multiList.InsertString(LIST_ITEM_ALL_INDEX,TALoadString(IDS_NAMEDFILTER_DEFAULT));
	...
	for (iter = m_listItems.begin(); iter != m_listItems.end(); iter++)
	{
		...
		index = m_multiList.InsertString( -1, TAUnicodeString::CreateFromMultitByteString(iter->str.c_str()));
