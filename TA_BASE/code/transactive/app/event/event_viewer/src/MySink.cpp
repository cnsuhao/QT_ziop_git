...
HRESULT CMySink::OnEventUpdate(long error, IATSEventList* pATSEvents)
{
 	...
 	if(error == 0 /*ATS_SUBSCRIPTION_OK*/)
	{
 		...
 		try{
			...
			ATSComWrapper* instance = dynamic_cast<ATSComWrapper*> (ATSComWrapper::getInstanceRealTime());
			instance->transformToISCSEvents ( pATSEvents, events, ATSRealTimeWorker::getCurrentMaxKey() );
