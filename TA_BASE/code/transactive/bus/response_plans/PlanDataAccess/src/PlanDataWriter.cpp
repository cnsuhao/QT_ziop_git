/**
  ...
  *  in any form.
  *
  *  Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/response_plans/PlanDataAccess/src/PlanDataWriter.cpp $
...
//		 strSql = database->prepareSQLStatement(STEP_DETAILES_SELECT_63551, nvlValue, planId);
		...
		database->prepareSQLStatement(strSql, STEP_PARAMETERS_SELECT_63051, nvlValue, planId);
         ...
         std::auto_ptr<TA_Base_Core::IData> paramData( database->executeQuery(strSql, paramCol));

         for( unsigned int unParamIndex = 0u; unParamIndex < paramData->getNumRows(); ++unParamIndex )
         {
             // Step position base on 1
             unsigned long ulStepPos = paramData->getUnsignedLongData( unParamIndex, "POSITION" ) - 1u;
             if ( ulStepPos >= auxPlanDetail->steps.length() )
             {
                 continue;
             }
             TA_Base_Core::AuxStepDetail& refStep = auxPlanDetail->steps[ulStepPos];
             unsigned long ulParamPos = paramData->getUnsignedLongData( unParamIndex, "PARAMETERPOS" );
             ...
             refStepParam.value = CORBA::string_dup( paramData->getStringData( unParamIndex, "PARAMETERVALUE" ).c_str() );
         }
