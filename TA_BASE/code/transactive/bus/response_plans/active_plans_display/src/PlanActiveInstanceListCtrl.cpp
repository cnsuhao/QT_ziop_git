/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/response_plans/active_plans_display/src/PlanActiveInstanceListCtrl.cpp $
...
#include "bus/generic_gui_view/src/TransactiveMessage.h" // TD14164 azenitha
...
namespace
{
    ...
    // Column labels
    const TCHAR *const PLAN_COLUMN_LABEL = _T("Plan");			    // Column 1 - Plan
    const TCHAR *const CATEGORY_COLUMN_LABEL = _T("Category");	    // Column 2 - Category
    const TCHAR *const INSTANCE_COLUMN_LABEL = _T("Instance");	    // Column 3 - Instance
    const TCHAR *const LOCATION_COLUMN_LABEL = _T("Location");       // Column 4 - Location
    const TCHAR *const OWNER_COLUMN_LABEL = _T("Owner");			    // Column 5 - Owner
    const TCHAR *const ACTIVATION_COLUMN_LABEL = _T("Activation");	// Column 6 - Activation time
    const TCHAR *const STATE_COLUMN_LABEL = _T("State");		    	// Column 7 - State
    const TCHAR *const STEP_COLUMN_LABEL = _T("Step");		    	// Column 8 - Step
    const TCHAR *const REMARKS_COLUMN_LABEL = _T("Remarks");		    // Column 9 - Remarks
...
void CPlanActiveInstanceListCtrl::ProcessInstance(const TA_Base_Core::ActivePlanDetail &activePlanDetail)
{
    ...
    *itemData = activePlanDetail;
	...
	TAUnicodeString currentStep = _T("");
    if (activePlanDetail.currentStepDetail.state != TA_Base_Core::UNDEFINED_ACTIVE_STEP_STATE)
    {
        ...
        TAUnicodeString stepName = TAUnicodeString::CreateFromMultitByteString( activePlanDetail.currentStepDetail.name.in() );
	    stepName.str().TrimRight();
	...
	SetItemText(position, OWNER_COLUMN_INDEX, (activePlanDetail.owner.in()));
    SetItemText(position, REMARKS_COLUMN_INDEX, (activePlanDetail.remark.in()));

    if (!changedOwner)
    {
        TAUnicodeString tempTAString = TAUnicodeString::CreateFromMultitByteString(activePlanDetail.path.in());

        TAUnicodeString planName = StringUtilities::extractPlanName(tempTAString); 

        CListCtrl::SetItemText(position, PLAN_COLUMN_INDEX, planName.GetString());

        tempTAString.fromMultiByteString(activePlanDetail.path.in());
	    CListCtrl::SetItemText(position, CATEGORY_COLUMN_INDEX, StringUtilities::extractCategoryPath(tempTAString).GetString());

	    CListCtrl::SetItemText(position, INSTANCE_COLUMN_INDEX, StringUtilities::convertToString(activePlanDetail.plan.instance).GetString());

        std::string locationNameStr = PlanAgentLocationAccess::getInstance().getLocationName(activePlanDetail.plan.location);
        tempTAString.fromMultiByteString( locationNameStr.c_str() );
        CListCtrl::SetItemText(position, LOCATION_COLUMN_INDEX,tempTAString.GetString() );

        tempTAString.fromMultiByteString( activePlanDetail.timeOfInstantiation.in() );
        CListCtrl::SetItemText(position, ACTIVATION_COLUMN_INDEX, tempTAString.GetString());

        CListCtrl::SetItemText(position, STATE_COLUMN_INDEX, StringUtilities::convertToString(activePlanDetail.activePlanState).GetString());
        CListCtrl::SetItemText(position, STEP_COLUMN_INDEX, currentStep.GetString());
    }
...
void CPlanActiveInstanceListCtrl::Refresh()
{
    ...
    try
    {
        ...
        for (LocationList::iterator iter = locations.begin(); iter != locations.end(); iter++)
        {
            ...
            catch (const TA_Base_Core::TransactiveException &ex) 
            {
				...
				//TAUnicodeString temp2 = ex.what();
				//std::string temp3 = PlanAgentLocationAccess::getInstance().getLocationName(location);
				//TAUnicodeString locationName = temp + temp2 + temp3.c_str();
            ...
            catch (const IPlanAgentCorbaDef::PlanAgentError &ex)
            {
				...
				//TA_Base_Bus::TransActiveMessage userMsg;
				//TAUnicodeString temp = "\n\n";
				//TAUnicodeString temp2 = ex.what.in();
				//std::string temp3 = PlanAgentLocationAccess::getInstance().getLocationName(location);
				//TAUnicodeString locationName = temp + temp2 + temp3.c_str();
				//userMsg << locationName;
				//UINT selectedButton = userMsg.showMsgBox(IDS_UE_210087);
            ...
            catch (const CORBA::Exception &ex)
            {
				...
				//std::string temp3 = PlanAgentLocationAccess::getInstance().getLocationName(location);
				//TAUnicodeString locationName = temp + temp2.c_str() + temp3.c_str();
				//userMsg << locationName;
				//UINT selectedButton = userMsg.showMsgBox(IDS_UE_210087);
				// TD14164 ++azenitha
				//++TD16971
    ...
    catch (const TA_Base_Core::TransactiveException &ex)
    {
        ...
        /*AfxMessageBox(_T(errorMessage.str().c_str()), MB_ICONERROR | MB_OK);*/
		TA_Base_Bus::TransActiveMessage userMsg;
        TAUnicodeString temp = TAUnicodeString::CreateFromMultitByteString(ex.what());
		TAUnicodeString error = _T("\n\n");
        error += temp;
		userMsg << error;
...
void CPlanActiveInstanceListCtrl::Purge()
{
    ...
    catch (const TA_Base_Core::TransactiveException &ex)
    {
		...
		TA_Base_Bus::TransActiveMessage userMsg;
        TAUnicodeString temp = TAUnicodeString::CreateFromMultitByteString(ex.what());
		TAUnicodeString actionName = _T("purge the active plan list. \n\n");
        actionName += temp;
		userMsg << actionName;
    ...
    catch (const IPlanAgentCorbaDef::PlanAgentError &ex)
    {
		...
		/*AfxMessageBox(_T(errorMessage.str().c_str()), MB_ICONERROR | MB_OK);*/
		TA_Base_Bus::TransActiveMessage userMsg;
        TAUnicodeString temp = TAUnicodeString::CreateFromMultitByteString(ex.what.in());
		TAUnicodeString actionName = _T("purge the active plan list. \n\n");
        actionName += temp;
		userMsg << actionName;
		UINT selectedButton = userMsg.showMsgBox(IDS_UW_632006);
    ...
    catch (const CORBA::Exception &ex)
    {
		...
		/*AfxMessageBox(_T(errorMessage.str().c_str()), MB_ICONERROR | MB_OK);*/
		TA_Base_Bus::TransActiveMessage userMsg;
		TAUnicodeString temp = _T("purge the active plan list. \n\n");
		std::string temp2 = TA_Base_Core::CorbaUtil::getInstance().exceptionToString(ex);
		TAUnicodeString actionName = temp;
        actionName += TAUnicodeString::CreateFromMultitByteString(temp2.c_str());
		userMsg << actionName;
		UINT selectedButton = userMsg.showMsgBox(IDS_UW_632006);
		// TD14164 ++azenitha
    }
...
void CPlanActiveInstanceListCtrl::InitialiseActivePlanList()
{
	if (!m_initialised) // want to do this once only
	{
		...
		//
		// Plan Column
		InsertColumn(PLAN_COLUMN_INDEX, (PLAN_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 180 : 0);
		// Category Column
		InsertColumn(CATEGORY_COLUMN_INDEX, (CATEGORY_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 180 : 0);
		// Instance Column
		InsertColumn(INSTANCE_COLUMN_INDEX, (INSTANCE_COLUMN_LABEL), LVCFMT_RIGHT, 65);
		// Location Column
		InsertColumn(LOCATION_COLUMN_INDEX, (LOCATION_COLUMN_LABEL), LVCFMT_LEFT, 70);
		// Owner Column 
		InsertColumn(OWNER_COLUMN_INDEX, (OWNER_COLUMN_LABEL), LVCFMT_LEFT, 150);
		// Activation Column 
		InsertColumn(ACTIVATION_COLUMN_INDEX, (ACTIVATION_COLUMN_LABEL), LVCFMT_LEFT, 135);
		// State Column
		InsertColumn(STATE_COLUMN_INDEX, (STATE_COLUMN_LABEL), LVCFMT_LEFT, 85);
		// Step Column
		InsertColumn(STEP_COLUMN_INDEX, (STEP_COLUMN_LABEL), LVCFMT_LEFT,	m_planPathVisible? 185 : 200);
		// Remarks Column
		// CL-14921++ Item 5
		//InsertColumn(REMARKS_COLUMN_INDEX, _T(REMARKS_COLUMN_LABEL), LVCFMT_LEFT, m_planPathVisible? 170 : 185); 
		InsertColumn(REMARKS_COLUMN_INDEX, REMARKS_COLUMN_LABEL, LVCFMT_LEFT, m_planPathVisible? 435 : 450); 
		// ++CL-14921
...
#ifdef _UNICODE
BOOL CPlanActiveInstanceListCtrl::SetItemText(int nItem, int nSubItem, const char * lpszText)
{
    TAUnicodeString tempString;
    tempString.fromMultiByteString(lpszText);
    return CListCtrl::SetItemText(nItem, nSubItem, tempString.GetString());
}
#endif
