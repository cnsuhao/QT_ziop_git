/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/response_plans/active_plans_display/src/ComparablePlanInstance.cpp $
  * @author:  Bart Golab
  * @version: $Revision: #1 $
  *
...
bool ComparablePlanInstance::operator==(const TA_Base_Core::ActivePlanDetail& activePlanDetail)
{
    ...
    thisActivationTime = StringUtilities::getDateTime(m_activePlanDetail.timeOfInstantiation.in());
    otherActivationTime = StringUtilities::getDateTime(activePlanDetail.timeOfInstantiation.in());
...
bool ComparablePlanInstance::operator<(const TA_Base_Core::ActivePlanDetail& activePlanDetail)
{
    ...
    {
        TAUnicodeString memberActivePlanPath = TAUnicodeString::CreateFromMultitByteString(m_activePlanDetail.path.in());
        TAUnicodeString activePlanPath = TAUnicodeString::CreateFromMultitByteString(activePlanDetail.path.in());

        std::string thisPlanName = StringUtilities::extractPlanName(memberActivePlanPath).toMultiByteStdString();
        std::string otherPlanName = StringUtilities::extractPlanName(activePlanPath).toMultiByteStdString();
        ...
        std::string thisCategoryPath = StringUtilities::extractCategoryPath(memberActivePlanPath).toMultiByteStdString();
        std::string otherCategoryPath = StringUtilities::extractCategoryPath(activePlanPath).toMultiByteStdString();
