/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/response_plans/plan_tree/src/PlanTreeCtrl.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
LRESULT CPlanTreeCtrl::OnSetSelectedPlan(WPARAM wParam, LPARAM lParam)
{
    ...
    try
    {
        ...
        for (IPlanAccess::PlanIdList::const_iterator iter = nodeIdsInPath.begin() + 1; iter != nodeIdsInPath.end(); iter++)
        {
			...
			TAUnicodeString tempUniItem = TAUnicodeString::CreateFromMultitByteString(searchKey.str().c_str());

		    treeItem->expandItem();
		    treeItem = treeItem->findSelectedItem(tempUniItem.c_str());
