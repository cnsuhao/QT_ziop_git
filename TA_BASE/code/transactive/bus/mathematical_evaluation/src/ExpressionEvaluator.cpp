/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mathematical_evaluation/src/ExpressionEvaluator.cpp $
  * @author:  Ross Tucker
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    :m_readCount(0), m_initialLen(0), m_contexts(), m_tokeniser()
    {
    }
    ...
    void ExpressionEvaluator::setNewExpression(const std::string& p_expression)
    {
        ...
        {
            m_tokeniser.setExpression(p_expression);
			m_initialLen = 0;	// reset, expression length mayby change.
        }
        catch( TA_Base_Core::MathematicalEvaluationException )
        {
            throw;
