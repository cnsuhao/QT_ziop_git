/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/sound/sound_client/src/SoundManagerProxy.cpp $
    ...
    void SoundManagerProxy::checkReturnValue(HRESULT hresult) const
    {
        if (FAILED(hresult))
        {
            ...
            if (errorInfo != NULL)
            {
                ...
                TAUnicodeString errorMsg;//TODO = TAUnicodeString::CreateFromBstrString(errorMsgBSTR);
                errorInfo->Release();
                TA_THROW(TA_Base_Core::SoundPlayerException(errorMsg.toMultiByteStdString()));
            }
            else
            {
