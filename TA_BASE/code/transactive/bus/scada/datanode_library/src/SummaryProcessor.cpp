/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/datanode_library/src/SummaryProcessor.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    {
        m_summaryProcessor->queueItem(item->getEntityKey(), item);
    ...
    {
        m_summaryProcessor->queueItemUnique(item->getEntityKey(), item);
	...
	void SummaryProcessor::queueProcessorPoolCallback( boost::shared_ptr<SummaryRequest> newRequest )
	{
		if (newRequest.get() != NULL)
		{
            ...
            {
			case SummaryRequest::ChildProxyUpdate:
                dataNode->processChildProxyUpdate(newRequest->getUpdateType());
                break;
			case SummaryRequest::ParentProxyUpdate:
				dataNode->processParentProxyUpdate(newRequest->getUpdateType());
				break;
