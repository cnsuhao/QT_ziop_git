/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/datanode_library/src/DataNode.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    m_persistence( persistence )
    {
		...
		{
			m_scadaRootKey = getScadaRootKey();
			TA_ASSERT(m_scadaRootKey != ULONG_MAX, "get ScadaRoot key error");
		}
        //
    ...
    void DataNode::processChildProxyUpdate(ScadaEntityUpdateType updateType)
    {
        std::ostringstream fnName;
        fnName << "processChildProxyUpdate() - " << m_staticConfig.name;
    ...
    void DataNode::processEntityUpdateEvent(unsigned long entityKey, ScadaEntityUpdateType updateType)
    {
		...
		SummaryRequest* pRequest = NULL;
		if (isInterestedParentUpdate(entityKey, updateType))
		{
			pRequest = new SummaryRequest(m_staticConfig.entityKey, updateType, SummaryRequest::ParentProxyUpdate);
		}
		else
		{
			pRequest = new SummaryRequest(m_staticConfig.entityKey, updateType, SummaryRequest::ChildProxyUpdate);
		}

        // Don't insert if this is in the queue already!
		boost::shared_ptr<SummaryRequest> request (pRequest);
    ...
    void DataNode::setInhibitMMS( const std::string& sessionID, bool inhibitMms )
    {
			  ...
			  (m_dynamicState.combinedInhibitMMSState.quality == QUALITY_GOOD_NO_SPECIFIC_REASON)))
		{
			if ( true == isActionPermitted ( sessionID, ( true == inhibitMms ? TA_Base_Bus::aca_DATA_NODE_MMS_INHIBIT : TA_Base_Bus::aca_DATA_NODE_MMS_UNINHIBIT ) ) )
			{
				...
				recalculateCombinedInhibitMMS(NotApplicable);
	...
	void DataNode::processParentProxyUpdate(ScadaEntityUpdateType updateType)
	{
		switch (updateType)
		{
		case CombinedInhibitMMSUpdate:
			recalculateCombinedInhibitMMS(updateType);
			break;
	...
	{
		unsigned long rootKey = ULONG_MAX;
		std::string scadaRootType = TA_Base_Core::ScadaRootEntityData::getStaticType();
		TA_Base_Core::IEntityDataList dataList = TA_Base_Core::EntityAccessFactory::getInstance().getEntitiesOfType(scadaRootType, false, false, false);
		if (0 < dataList.size())
		{
			TA_Base_Core::ScadaRootEntityData* pData = dynamic_cast<TA_Base_Core::ScadaRootEntityData*>(dataList[0]);
			if (pData != NULL)
			{
				rootKey = pData->getKey();
			}
		}
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "get scada root key:%d", rootKey);
	...
	{
		std::ostringstream fnName;
		fnName << "submitParentUpdateEvent() - " << m_staticConfig.name;
		FUNCTION_ENTRY( fnName.str().c_str() );
	...
	void DataNode::recalculateCombinedInhibitMMS( ScadaEntityUpdateType type )
	{
			...
			preCombineMMS.value != m_dynamicState.combinedInhibitMMSState.value)
		{
			...
			SummaryRequest* pRequest = new SummaryRequest(m_staticConfig.entityKey, MmsAlarmInhibitSummaryUpdate, SummaryRequest::ChildProxyUpdate);
			boost::shared_ptr<SummaryRequest> request (pRequest);
			m_summaryQ->queueItemUnique(request);
		}
		...
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, 
				"%d process parent update,updateType:%d-noitify:%d-MMSState:%d-Combinedvalue:%d-CombineQuality:%d", 
				m_staticConfig.entityKey, type, isNotify, 
				m_dynamicState.inhibitMMSState,
				m_dynamicState.combinedInhibitMMSState.value, 
				m_dynamicState.combinedInhibitMMSState.quality);
		}
		...
		{
			LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, 
				"%d process parent update,updateType:%d-noitify:%d-parentCombineValue:%d-ParentCombineQuality:%d-MMSState:%d-Combinedvalue:%d-CombineQuality:%d", 
				m_staticConfig.entityKey, type, isNotify, 
	...
	bool DataNode::isInterestedParentUpdate(unsigned long entitykey, ScadaEntityUpdateType type)
	{
		if (entitykey == m_parentKey &&
			(CombinedInhibitMMSUpdate == type || 
			ConfigAvailable == type))
		{
			return true;
		}
		else
