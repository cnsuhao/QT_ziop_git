/**
  ...
  * in any form.
  *
  * Source : $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/datapoint_library/src/DataPointFactory.cpp $
  * @author  J. Welton
  * @version $Revision: #1 $
...
namespace TA_Base_Bus
{
												 ...
												 const std::string & associations )
	{
		...
		if ( false == associations.empty() )
		{
			...
			while ( currentPos < associations.length() )
			{
				...
				// Locate the next ':' and ';'
				//
				unsigned int colonPos = associations.find( ":", currentPos );
				unsigned int semiColonPos = associations.find( ";", currentPos );
    ...
    void DataPointFactory::setToMonitor()
    {
        ...
        // to initlalize mms global inhibition.
        m_mmsScadaUtility.isMmsGlobalInhibited();
    }
