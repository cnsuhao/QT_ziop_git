/**
  ...
  * any form.
  *
  * Source:		$File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/datapoint_library/src/DataPoint.cpp $
  * @author:	HoaVu
  * @version $Revision: #1 $
  *
		...
		if ( true == deviceRequestToBeSubmitted )
		{
			...
			if ( true == evaluateLaunchCondition() )
			{
                ...
                if ( true == isSystemSessionID( sessionID) ) //limin++, SystemSession should not access db
                {
                    ...
                    m_persistence->writePersistedValue( DP_CURRENT_CONTROL_REQUEST, m_entityKey, currentRequest.str(), m_fieldValueQualityStatus );
                }
			}
			else
			{
	...
	void DataPoint::saveLastOperatorInfo ( const std::string & sessionID, const timeb & timestamp )
	{
		...
		if ( TA_Base_Core::Control == m_operationMode )
		{
            ...
            if (sessionID != DPT_INTERNAL_SESSION_ID)
            {
                ...
                {
                    m_persistence->writePersistedValue( DP_LAST_OPERATOR_SESSION_ID, m_entityKey, sessionID, m_fieldValueQualityStatus );
                }

			    // Update database with the new timestamp
                if (lastTimestamp.time != timestamp.time || lastTimestamp.millitm != timestamp.millitm)
                {
                    m_persistence->writePersistedValue( DP_LAST_OPERATOR_TIMESTAMP, m_entityKey, m_lastOperatorTimestamp.time, m_fieldValueQualityStatus );
                }
            }
