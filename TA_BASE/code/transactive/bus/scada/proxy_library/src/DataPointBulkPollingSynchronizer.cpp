/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/proxy_library/src/DataPointBulkPollingSynchronizer.cpp $
...
namespace TA_Base_Bus
{
    ...
    {
        LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug, "Creating DataPointBulkPollingSynchronizer for %s", agentName.c_str() );
    ...
    void DataPointBulkPollingSynchronizer::timerExpired(long timerId, void* userData)
    {
        m_timerSemaphore.post();
    }
    ...
    void DataPointBulkPollingSynchronizer::queryAgentInterface()
    {
        ...
        LOG_GENERIC(SourceInfo, DebugUtil::DebugTrace, "%s start poll, %d new items", m_agentName.c_str(), m_addList.size());
        ...
        try
        {
            ...
            {
                LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "%s skip poll, m_corbaErrorPollSkipCount[%d], %d new items", m_agentName.c_str(), m_corbaErrorPollSkipCount, m_addList.size());
    ...
    // throws. All exceptions caught by the calling function
    {
		...
		LOG_GENERIC( SourceInfo, DebugUtil::DebugDebug,
            "Observer ID %u received %u data point updates from data point agent %s",
    ...
    void DataPointBulkPollingSynchronizer::addEntity ( const RequestItem & requestItem )
    {
        ...
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "DataPointBulkPollingSynchronizer AddDataPoint %s:%u",
    ...
    void DataPointBulkPollingSynchronizer::removeEntity ( const RequestItem & requestItem )
    {
        ...
        if (itr != m_syncEntities.end())
        {
            LOG_GENERIC( SourceInfo, DebugUtil::DebugInfo, "DataPointBulkPollingSynchronizer RemoveDataPoint %s:%u",
                getAgentName().c_str(), requestItem.entityKey );
    ...
    void DataPointBulkPollingSynchronizer::setToBadNotConnectedQuality ( TA_Base_Bus::ExceptionType exception )
    {
        ...
        switch ( exception )
        {
            ...
            default:
                ...
                if (m_isConnected && time(NULL) > m_timeLastConnected + SET_BAD_QUALITY_RETRY_PERIOD_SECS)
                {
					m_isConnected = false;
                    // for each synchronized entity, set proxy to agent not avaiable
                    for (std::set<unsigned long>::iterator it = m_syncEntities.begin();
