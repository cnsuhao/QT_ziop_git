/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/scada_tree/src/EntityTreeItem.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
{
    TAUnicodeString tempTaStr = TAUnicodeString::CreateFromMultitByteString( entityData->getName().c_str() );
    m_entityLabel = getDisplayName(tempTaStr);
...
TAUnicodeString CEntityTreeItem::getDisplayName(const TAUnicodeString& fullName)
{
	TAUnicodeString newName(fullName);

    int position = fullName.c_str().ReverseFind(_T('.'));
	if (position!=-1)
	{
...
const TCHAR* CEntityTreeItem::getItemLabelText()
{
	return m_entityLabel.GetString();
}


const std::string CEntityTreeItem::getKey()
{
	return m_entityLabel.toMultiByteStdString();
}
