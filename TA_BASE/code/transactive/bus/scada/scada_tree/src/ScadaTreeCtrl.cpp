/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/scada/scada_tree/src/ScadaTreeCtrl.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
void CScadaTreeCtrl::setInitialDataPoint(std::string dataPointName)
{
	...
	try
	{
		...
		if (!dataPointPath.empty())
		{
            ...
            TAUnicodeString keyString;
            keyString.fromMultiByteString(key.c_str());
			treeItem = findSelectedRootItem(keyString.GetString());
			...
			while ((treeItem!=NULL)&&(!dataPointPath.empty()))
			{
				...
				treeItem = treeItem->findSelectedItem(keyString.GetString());
			}
