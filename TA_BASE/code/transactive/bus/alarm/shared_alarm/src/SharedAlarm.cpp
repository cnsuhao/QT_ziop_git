...
//              >());
    ...
    void TA_SharedAlarm::notifyClient(const char* name)
    {
...
//  }
    ...
    int updateAlarmByID(alarmindex_by_id::iterator& itFind, const AlarmDetail& alarmUpdate)
    {
        TA_Base_Core::AlarmItem* pFindAlarm =  *itFind;
        ...
        try
        {
            ...
            // ************************************ N O T E *********************************//
            // if the existing alarm is normalize and we received an update alarm with an
            // Open or Acknowledge state we need to delete the existing alarm as the alarm
            // has been raised again. The new alarm will be added by the calling function.
            // ******************************************************************************//
            if (alarmUpdate.state == I_ATS_ISCS_ALARM::AlarmClosed ||
                    ((alarmUpdate.state == I_ATS_ISCS_ALARM::AlarmOpen ||
                      alarmUpdate.state == I_ATS_ISCS_ALARM::AlarmAcknowledged) &&
                     pFindAlarm->state == TA_Base_Core::AlarmClosed))
            {
                ...
                for (int i = 0; i < g_pShAlarmInfo->clients; ++ i)
                {
                    if (alarmIndexWithFilter[i].index != NULL)
                    {
                        AlarmIndex::iterator it2 = alarmIndexWithFilter[i].index->find(const_cast<char*>(alarmUpdate.alarmID.c_str()));
            ...
            {
                LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "update ats alarm");

                TA_Base_Core::AlarmUpdateCorbaDef alarmUpdateCorba;

                alarmUpdateCorba.ackTime = alarmUpdate.ackTime;
                alarmUpdateCorba.alarmAcknowledgedBy = CORBA::string_dup(alarmUpdate.alarmAcknowledgeBy.c_str());
                alarmUpdateCorba.alarmComments = CORBA::string_dup("");
                alarmUpdateCorba.alarmDescription = CORBA::string_dup(alarmUpdate.alarmDescription.c_str());

                //alarmUpdate.state==I_ATS_ISCS_ALARM::AlarmClosed
                if (alarmUpdate.ackTime != 0)
                {
                    alarmUpdateCorba.alarmHasBeenAcknowledged = CORBA::Boolean(true);
                    alarmUpdateCorba.alarmHasBeenClosed = CORBA::Boolean(false);
                    alarmUpdateCorba.alarmHasBeenOpened = CORBA::Boolean(false);

                    if (alarmUpdate.alarmAcknowledgeBy.empty())
                    {
                        //alarmUpdateCorba.alarmAcknowledgedBy=CORBA::string_dup("c952");
                        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugError, "ATS Alarm: Acknowledge time is set=%ld, but AcknowledgeBy is Empty.", alarmUpdate.ackTime);
                    }
                }
                else if (alarmUpdate.ackTime == 0)
                {
                    alarmUpdateCorba.alarmHasBeenAcknowledged = CORBA::Boolean(false);

                    if (alarmUpdate.state == I_ATS_ISCS_ALARM::AlarmReturned)
                    {
                        alarmUpdateCorba.alarmHasBeenClosed = CORBA::Boolean(true);
                        alarmUpdateCorba.alarmHasBeenOpened = CORBA::Boolean(false);

                        if (0 == alarmUpdate.closeTime)
                        {
                            alarmUpdateCorba.closeTime = alarmUpdate.sourceTime.time;
                        }
                        else
                        {
                            alarmUpdateCorba.closeTime = alarmUpdate.closeTime;
                        }

                        LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "closeTime is [%ul], sourceTime is [%ul]", alarmUpdate.closeTime, alarmUpdate.sourceTime.time);
                    }
                    else
                    {
                        alarmUpdateCorba.alarmHasBeenClosed = CORBA::Boolean(false);
                        alarmUpdateCorba.alarmHasBeenOpened = CORBA::Boolean(true);
                    }
                }

                alarmUpdateCorba.alarmID = CORBA::string_dup(alarmUpdate.alarmID.c_str());
                alarmUpdateCorba.assetName = CORBA::string_dup(alarmUpdate.assetName.c_str());
                alarmUpdateCorba.assocEntityKey = 0;
                alarmUpdateCorba.closeValue = CORBA::string_dup(alarmUpdate.alarmValue.c_str());
                alarmUpdateCorba.commentsWereAdded = false;
                alarmUpdateCorba.decisionSupportState = TA_Base_Core::NoRelatedPlan;
                alarmUpdateCorba.decisionSupportStateHasChanged = CORBA::Boolean(false);
                alarmUpdateCorba.locationKey = 2;
                alarmUpdateCorba.messageContext = CORBA::string_dup("");
                alarmUpdateCorba.messageSequenceNumber = 0;
                alarmUpdateCorba.messageTypeKey = 0;

                alarmUpdateCorba.mmsState = (TA_Base_Core::MmsStateType) alarmUpdate.mmsState;
                alarmUpdateCorba.mmsStateHasChanged = CORBA::Boolean(true);

                alarmIndexWithDuty->modify(itFind, updateAlarmDetailATS(alarmUpdateCorba));
                ...
                for (int i = 0; i < g_pShAlarmInfo->clients; ++ i)
                {
                    if (alarmIndexWithFilter[i].index != NULL)
                    {
                        ...
                        {
                            alarmIndexWithFilter[i].index->modify(filteritor, updateAlarmDetailATS(alarmUpdateCorba));
    ...
    void TA_SharedAlarm::insertOrUpdateAlarm(std::vector<AlarmDetail*>& atsAlarms, bool isPassPreFilter)
    {
        ...
        for (; iterATSAlarm != atsAlarms.end(); iterATSAlarm++)
        {
            ...
            try
            {
                iRet = updateAlarmByID(itFind, *atsAlarmDetail);

                // **************************************** NOTE ************************************** //
                // iRet == -1 means existing alarm has been removed. If existing alarm is removed and the
                // ATS alarm update state is AlarmOpen or AlarmAcknowledged this means that alarm has been
                // reopened and we need to insert the new alarm to duty index.
                // Please do note that the code below cannot be move inside updateAlarmByID function because
                // the function is not a member of TA_SharedAlarm class.
                //**************************************************************************************//
                if (-1 == iRet && atsAlarmDetail->state == I_ATS_ISCS_ALARM::AlarmOpen ||
                        atsAlarmDetail->state == I_ATS_ISCS_ALARM::AlarmAcknowledged)
                {
                    LOG_GENERIC(SourceInfo, TA_Base_Core::DebugUtil::DebugInfo, "ATS alarm re-opened with ID %s", atsAlarmDetail->alarmID.c_str());
                    addAlarmItem(*atsAlarmDetail, isPassPreFilter);
                }
            }
            catch (...)
