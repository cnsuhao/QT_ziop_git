...
//					m_pSharedAlarm->updateIndexInfo(m_indexInfo, m_csPos);
	...
	void CSimpleDispMultiIndex::setDisplaySort(SORT_COLUMN sortType, bool bSortAccending)
	{
		...
		LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "before calling getListDisplayData from setDisplaySort");
		// Refresh the cache
		getListDisplayData();
	...
	void CSimpleDispMultiIndex::setListDisplayInfo(ListDispInfo& listInfo)
	{
		...
		LOG0(SourceInfo, TA_Base_Core::DebugUtil::DebugDebug, "before calling getListDisplayData from setListDisplayInfo");
		getListDisplayData();
	}
