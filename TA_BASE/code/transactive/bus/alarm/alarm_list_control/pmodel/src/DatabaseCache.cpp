/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/bus/alarm/alarm_list_control/src/DatabaseCache.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    unsigned long DatabaseCache::getAlarmSeverityColour(unsigned long key, TA_Base_Core::IAlarmSeverityData::EColourType type)
    {
        if (m_alarmSeverities.empty())
        {
            loadAlarmSeverities();
        }
		unsigned long colorRefRetVal;
        unsigned long defaultForegroundColour =0;
        unsigned long defaultBackgroundColour = 255*255*255;
