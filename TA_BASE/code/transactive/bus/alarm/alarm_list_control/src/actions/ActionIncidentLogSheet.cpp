 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionIncidentLogSheet.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Bus
{
    ...
    void ActionIncidentLogSheet::performAction(IAlarmRetriever& alarmRetriever)
    {
		...
		TAUtfString strAlarmValue = alarm->alarmValue.c_str();
		strAlarmValue.replace(TAUtfString(" "), TAUtfString("{%Blank%}"));	// replace blank in planpath with {%Blank%} to accept the Plan Categories with blanks, 28.Mar.2007, hongzhi
		commandLine << "  --alarm-value=" << strAlarmValue.std_str();
		...
		try
		{
			TA_Rect position = TA_Base_Bus::AppLauncher::getInstance().getRect(SCREEN_CURRENT,
				AREA_SCHEMATIC,
				pt.x());
