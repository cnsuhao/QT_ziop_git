 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionSuppressAlarm.h $
 * @author:  Anita Lee
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Bus
{
    ...
    {
		Q_DECLARE_TR_FUNCTIONS(ActionSuppressAlarm)
    public:
		...
		virtual std::map<unsigned int, QString> getSubActionControlIds()
		{
			std::map<unsigned int, QString> subActionIds;
			subActionIds.insert(std::pair<unsigned int, QString>(INHIBIT_ALARM_BY_TYPE, tr("By T&ype")));
			subActionIds.insert(std::pair<unsigned int, QString>(INHIBIT_ALARM_BY_DESCRIPTION, tr("By Descripti&on")));
			return subActionIds;
		};
