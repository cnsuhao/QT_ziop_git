 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionRelatedDSS.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Bus
{
    ...
    void ActionRelatedDSS::performAction(IAlarmRetriever& alarmRetriever)
    {
		...
		ITransActiveView *pView = m_model->getWinApp()->getViewManager()->getView(ViewType_PlanAssociation, "");
		if (IDOK == pView->doModal())
		{
			PlanAssociationDialog* pPlanAssocDlg = dynamic_cast<PlanAssociationDialog*>(pView);
			std::ostringstream commandLine;
			commandLine << "--exec-plan-id=" << pPlanAssocDlg->getSelectedPlanPath();
