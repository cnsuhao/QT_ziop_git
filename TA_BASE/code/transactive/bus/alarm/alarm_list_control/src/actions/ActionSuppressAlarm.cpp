 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionSuppressAlarm.cpp $
 * @author:  Anita Lee
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Bus
{
    ...
    void ActionSuppressAlarm::performAction(IAlarmRetriever& alarmRetriever)
    {
		...
		if (canPerform)
		{
			...
			ITransActiveView* view = m_model->getWinApp()->getViewManager()->getView(ViewType_Suppression, "");
			SuppressAlarmDialog* dlg = dynamic_cast<SuppressAlarmDialog*>(view); //(m_pAlarmDetails, m_sessionKey, m_profileKey, m_operatorName);
			dlg->setAlarmDetails(m_pAlarmDetails, m_sessionKey, m_profileKey, m_operatorName);
			boost::shared_ptr<bool> result(new bool);
