 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/actions/ActionAcknowledgeAlarm.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #3 $
 *
...
namespace TA_Base_Bus
{
    ...
    void ActionAcknowledgeAlarm::performAction(IAlarmRetriever& alarmRetriever)
    {
		...
		//QMessageBox::warning(NULL,QObject::tr("aaa"), "ccc", QMessageBox::Ok,	QMessageBox::NoButton);
        ...
        alarmRetriever.setIsActionAlreadyRunning(true, tr("Acknowledging alarms . . ."));
		...
		for (std::vector<std::string>::iterator itr = selectedAlarms.begin(); itr != selectedAlarms.end(); itr++)
		{
			...
			if (bool(alarm->state == TA_Base_Core::AlarmOpen || alarm->state == TA_Base_Core::AlarmClosed))
			{
				...
				else
				{
					...
					{
						ITransActiveView* preconflictAckDlg = m_model->getWinApp()->getViewManager()->getView(ViewType_PreConflict, "");
						PreConflickDialog* pTemp = dynamic_cast<PreConflickDialog*>(preconflictAckDlg);
						pTemp->setAlarmDetail(alarm.get());
						
						if (QDialog::Accepted == preconflictAckDlg->doModal())
						{
		...
		try
		{
			if (tempselectalarm.size()>0)
			{
				...
				m_alarmHelper.operatorAcknowledgeAlarmsEx(tempselectalarm, alarmNoExists);		//xinyang++ CL-20990
