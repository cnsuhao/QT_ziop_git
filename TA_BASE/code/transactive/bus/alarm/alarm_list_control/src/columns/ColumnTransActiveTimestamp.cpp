 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/columns/ColumnTransActiveTimestamp.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #2 $
 *
...
namespace TA_Base_Bus
{
    ...
    std::string ColumnTransActiveTimestamp::getItemText(TA_Base_Core::AlarmDetailCorbaDef* alarm, DatabaseCache& databaseCache)
    {
        ...
        {
            std::string value;
            std::string time(COleDateTime(transactiveTime.time).Format(m_formatStr.operator LPCTSTR()));
            time += ".%03d";
     ...
     */
    void ColumnTransActiveTimestamp::setTimeFormat(const std::string& formatStr)
	{
