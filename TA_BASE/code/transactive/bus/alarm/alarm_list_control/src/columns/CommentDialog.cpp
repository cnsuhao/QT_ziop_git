/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI/TA_BASE/transactive/bus/alarm/alarm_list_control/src/CommentDialog.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #3 $
  *
  * Last modification: $DateTime: 2014/03/10 22:16:18 $
		...
		else if (m_selectedAlarmDetails.size() > 1)// Error - no alarm details for selected alarms
		{
			...
			for (int i=0; i < static_cast<int> (m_selectedAlarmDetails.size()) ; ++i)
			{
				...
				if (editEnabled == false) {
					...
					TA_Base_Bus::TransActiveMessage userMsg;
					userMsg << tempString;
					TAUnicodeString errMsg = userMsg.constructMessage(IDS_UW_050018);
