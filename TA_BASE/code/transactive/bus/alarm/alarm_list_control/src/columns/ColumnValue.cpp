 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/alarm/alarm_list_control/src/columns/ColumnValue.cpp $
 * @author:  Karen Graham
 * @version: $Revision: #4 $
 *
...
namespace TA_Base_Bus
{
	...
	const TAUtfString ColumnValue::getItemText(unsigned long ulIndex)
    {
		// The value will be: "AlarmType Value"
		/*
        std::string text = alarm->alarmValue;

		//TD 13146 Check for the temperature and '?' and replace '?' with '?
		std::string alarmDesc = alarm->alarmDescription;
		alarmDesc.MakeUpper();
		...
		*/
		try
		{
			return TAUtfString::createFromMultitByteString(m_model->getDisplayDataWrap()->getItemString(ulIndex, ALARM_VALUE));
		}
		catch(...)
		{ 
