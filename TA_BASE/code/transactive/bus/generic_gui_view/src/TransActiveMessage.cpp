...
namespace TA_Base_Bus
{
    ...
    int TransActiveMessage::showMsgBox(int uiMessageID,  std::string caption)
    {
        /*std::string pszMessage="abc";
        int uiMsgBoxType=0;*/
        int ret = 0;

        if (QThread::currentThread() == QApplication::instance()->thread())
        {
            doMsgBox(uiMessageID, caption, &ret, false);
        }
        else
        {
            this->moveToThread(QApplication::instance()->thread());
            //make sure the view will be created and register before return.
            QMetaObject::invokeMethod(this, "doMsgBox", Qt::BlockingQueuedConnection,
                                      Q_ARG(int, uiMessageID),
                                      Q_ARG(std::string, caption),
                                      Q_ARG(int*, &ret),
                                      Q_ARG(bool, false));
        }
    ...
    void TransActiveMessage::doMsgBox(int uiMessageID,  std::string caption, int* ret, bool bAuditEvent)
    {
        ...
        if (m_config != NULL)
        {
            ...
            // TODO: audit event
            *ret = convertResultFromQtToWindows(msg.exec());
        }
    }
