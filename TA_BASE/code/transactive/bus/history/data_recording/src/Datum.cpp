 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/history/data_recording/src/Datum.cpp $
 * @author:  Jade Welton
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Bus
{
    ...
    {
        m_value.m_string = new char[ value.length() + 1 ];
    ...
    : m_name( name ), m_type( STRING_ITEM )
    {
        m_value.m_string = new char[ strlen(value) + 1 ];
        ...
        if ( &copyMe != this )
        {
            switch( copyMe.m_type )
            {
                ...
                case STRING_ITEM :
                    ...
                    m_value.m_string = new char[ strlen( copyMe.m_value.m_string ) + 1 ];
