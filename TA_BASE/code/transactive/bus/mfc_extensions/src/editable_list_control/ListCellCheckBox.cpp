/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/editable_list_control/ListCellCheckBox.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
    ...
    m_parentUpdated(false)
{
    ...
    Create(_T(""), WS_CHILD | BS_AUTOCHECKBOX, boxRect, m_parent, IDC_LIST_EDIT_CTRL);
    
    SetCheck(text == _T("Yes") ? 1 : 0);
    SetFocus();
...
void CListCellCheckBox::Update() 
{
	...
	else if (!m_parentUpdated)
	{
        ...
        TAUnicodeString str = GetCheck() == 1? _T("Yes") : _T("No");

        if (m_cellManager->UpdateCellValue(m_parent, m_item, m_subItem, str))
        {
		    m_parent->SetItemText(m_item, m_subItem, str.str());
		    ...
		    dispinfo.item.pszText =  LPTSTR((LPCTSTR)str.str());
