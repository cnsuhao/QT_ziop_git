/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/print_list_ctrl/ListCtrlPrint.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
bool CListCtrlPrint::OnPreparePrinting(CPrintInfo* pInfo, bool bPrintPreview)
{
	...
	pColTitle = new TCHAR *[iNumCol];
	ZeroMemory(pColTitle, sizeof(TCHAR *) * iNumCol);
	...
	for (unsigned int t = 0; t <m_colToHide.size(); t++)
	{
		...
		if (i < iNumCol )
		{
			...
			if (pHeader->GetItem(t, &hi))
			{
				...
				{
					pColTitle[i] = new TCHAR[hi.cchTextMax];
					ASSERT(pColTitle[i]);
	...
	while (anySpace)
	{
		...
		else // there is no more space
		{
			...
			else if ( remainText.GetLength() > charsWide )
			{
				...
				remainText.Insert( charsWide, _T(" ") );
				if (addedSpace==false) addedSpace = true;
			}
			else
			{
...
CSize CListCtrlPrint::PrintListHead(CDC * pDC, CRect & rcPage, bool bCalc)
{
	if (!bCalc)
	{
		...
		for (int t = 0; t < iNumCol; t++)
		{
			...
			CString cs;
			cs.Format(_T(" %s"),pColTitle[t] ? pColTitle[t] : _T(""));
			//pDC->Rectangle(rc1);
