/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/numeric_edit_control/numericedit.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
void CNumericEdit::OnChar(UINT nChar, UINT nRepCnt, UINT nFlags)
{
    ...
    if (nChar != _T('\b') && (m_minimum.isSet || m_maximum.isSet))
    {
        ...
        if (newText != _T('.') && newText != _T('-'))
        {
            double value = _tstof(newText);
