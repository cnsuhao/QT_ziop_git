...
/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/grid_control/GridCtrlS.cpp $
        ...
        || (m_MouseMode == MOUSE_NOTHING && (nFlags & MK_LBUTTON)))
    {
        ...
        if (m_MouseMode == MOUSE_NOTHING && m_bTitleTips)
        {
                ...
                GetCellRect(idCurrentCell.row, idCurrentCell.col, CellRect))
			{
				...
				{
					if (text.Compare(_T("")) != 0) // base actionGroup is not empty
					{
...
BOOL CGridCtrlS::DrawCell(CDC* pDC, int nRow, int nCol, CRect rect, BOOL bEraseBk)
{
	...
	if (Item.bOverrided)
	{
		if (Item.szText.Compare(_T("")) != 0) // base actionGroup is not empty
		{
