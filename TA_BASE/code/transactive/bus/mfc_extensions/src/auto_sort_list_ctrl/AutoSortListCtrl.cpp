/**
  ...
  * in any form.
  *
  * Source: $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/auto_sort_list_ctrl/AutoSortListCtrl.cpp $
  * @author Jade Lee
  * @version $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
    ...
    static const DWORD WM_REAPPLY_SORT = RegisterWindowMessage(_T("AutoSortListCtrl_reSort"));
    ...
    sortFunction(LPARAM lParam1, LPARAM lParam2, LPARAM lParamSort)
    {
        ...
        const TCHAR* str1 = (LPCTSTR)info->columnData.at(lParam1);
        const TCHAR* str2 = (LPCTSTR)info->columnData.at(lParam2);
		...
		case AutoSortListCtrl::BY_MIXED:
		{
			...
			if (a1 == 0 && a2 == 0) 
			{
				result = _tcsicmp(str1, str2);
		
				if (0 == result)
				{
					// Go to case sensitive when strings the same
					result = -_tcscmp(str1, str2);
				}
			}
			else if (a1 != 0 && a2 != 0)
			{
...
#endif
	            headerItem.hbm = (HBITMAP)LoadImage(GetModuleHandle(dllName),
