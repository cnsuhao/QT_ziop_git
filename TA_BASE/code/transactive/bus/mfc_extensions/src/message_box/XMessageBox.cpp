/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/message_box/XMessageBox.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
HWND         ghWndMessageBox;			// handle of message box
...
#ifdef _DEBUG
#ifdef _UNICODE
    const TCHAR * dllName = _T("TA_MFCExtensions_du.dll");
#else
    const TCHAR * dllName = _T("TA_MFCExtensions_d.dll");
#endif //UNICODE
#else  //release
#ifdef _UNICODE
    const TCHAR * dllName = _T("TA_MFCExtensions_u.dll");
#else
    const TCHAR * dllName = _T("TA_MFCExtensions.dll");
#endif //UNICODE

#endif

    HINSTANCE hModInst = GetModuleHandle(dllName);
...
int CMbDialogTemplate::Display(LPCTSTR lpszCaption)
{
...
#else
    int nTitleLen = _tcslen(lpszCaption);
#endif
	...
	TRY  // catch memory exceptions and don't worry about allocation failures
	{
		...
		*(WORD*)(pdest + 1) = 0;                        // use default window class
		...
		delete pchCaption;
#else
        nActualChars = nTitleLen+1;
        memcpy(pdest, lpszCaption, nActualChars * sizeof(WCHAR));
        pdest += nActualChars * sizeof(WCHAR);
#endif 
		...
		for (i = 0; i < gdlgTempl.cdit; i++)
		{
			...
			*(WORD*)pdest = (WORD)m_pDlgItemArray[i]->m_controltype;     // atom value for the control
			...
			delete pchCaption;
#else
            int nActualChars = _tcslen(m_pDlgItemArray[i]->m_szCaption)+1;
            ASSERT(nActualChars > 0);
            memcpy(pdest, m_pDlgItemArray[i]->m_szCaption, nActualChars * sizeof(WCHAR));
            pdest += nActualChars * sizeof(WCHAR);
