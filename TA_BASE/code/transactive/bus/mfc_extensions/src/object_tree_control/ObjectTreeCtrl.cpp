/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/object_tree_control/ObjectTreeCtrl.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
void CObjectTreeCtrl::onGetdispinfo(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if (this->IsWindowEnabled())
	{
		...
		if ((pTVDispInfo->item.mask & TVIF_TEXT) == TVIF_TEXT)
		{
            ...
            int nTextLength = _tcslen(treeItem->getItemLabelText());
            TCHAR *buffer = new TCHAR [nTextLength+1];
            buffer[0] = 0x0;
            _tcscpy_s(buffer,nTextLength+1, treeItem->getItemLabelText());
			pTVDispInfo->item.pszText = buffer;
		}
...
CObjectTreeItem* CObjectTreeCtrl::findSelectedRootItem(const TCHAR* szKey)
{
	...
	while (hItem!=NULL)
	{
        ...
        CString cstrKey = treeItem->getKey().c_str();
		if ( cstrKey.Compare(szKey) == 0 ) break;
