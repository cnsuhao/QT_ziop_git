/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/mfc_extensions/src/object_tree_control/ObjectTreeItem.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
CObjectTreeItem* CObjectTreeItem::findSelectedItem(const CString& item, EItemSearchType searchType)
{
	...
	while (hItem != NULL)
	{
        ...
        {
            case IST_BY_KEY:
                //??? if the key is only english literal, then ok
                treeItemId = treeItem->getKey().c_str();
                break;
...
const TCHAR* CObjectTreeItem::getItemLabelText()
{
	// AJPTODO: Demo for now, make pure
	return _T("Test");
}
