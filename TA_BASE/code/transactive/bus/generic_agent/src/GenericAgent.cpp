/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/bus/generic_agent/src/GenericAgent.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Bus
{
          ...
          m_hostname(TA_Base_Core::Hostname::getHostname())
    {
        try
        {
            ...
            //xinsong move it into UtilityInitialiser::initialiseUtilities
            /*
    ...
    void GenericAgent::run()
    {
                ...
                m_agentStatus == TA_Base_Core::RunningMonitor)
        {
                    ...
                    (offlineGroups.compare(GenericAgent::GROUP_ONLINE) != 0))
            {
                ...
                int startPos = 0;
                int commaPos = offlineGroups.find_first_of(",", startPos);
    ...
    void GenericAgent::fillNSADataWithoutDB()
    {
        ...
        TA_Base_Core::IEntityDataPtr smartEntityDataPtr(entityData);
        m_agentEntityData = smartEntityDataPtr;

        ///construct the child entities
        const int max_channel = 10;
        NSAChannelData channelData[max_channel];
        channelData[0].channelContext = "Alarm";
        channelData[0].pkey = 20000034;
        channelData[1].channelContext = "Audit";
        channelData[1].pkey = 20000035;
        channelData[2].channelContext = "StateUpdate";
        channelData[2].pkey = 20000036;
        channelData[3].channelContext = "DataPoint";
        channelData[3].pkey = 20000037;
        channelData[4].channelContext = "DataNode";
        channelData[4].pkey = 20000038;
        channelData[5].channelContext = "GeneralComms";
        channelData[5].pkey = 20000039;
        channelData[6].channelContext = "NameBind";
        channelData[6].pkey = 20000040;
        channelData[7].channelContext = "Config";
        channelData[7].pkey = 20000041;
        channelData[8].channelContext = "BroadcastComms";
        channelData[8].pkey = 20000042;
        channelData[9].channelContext = "InnerMonitor";
        channelData[9].pkey = 20000043;

        TA_Base_Core::SharedIEntityDataList childEntityData;

        for (int i = 0; i < max_channel; i++)
        {
            int channelId = channelData[i].pkey;
            TA_Base_Core::EntityData* entityData = new TA_Base_Core::DefaultEntity(channelId, "NotifyEventChannel");
            TA_Base_Core::EntityHelper* pHelper = entityData->getHelper();

            pHelper->setIsValidData();

            pHelper->setName("unknownchannelname");

            pHelper->setAddress(channelData[i].channelContext);
            childEntityData.push_back(TA_Base_Core::IEntityDataPtr(entityData));
        }
    ...
    void GenericAgent::doEntityConfiguration(const unsigned int agentUserType)
    {
        try
        {
            ...
            bool fTestAlone = RunParams::getInstance().isSet("LocalTest");

            if (fTestAlone)
            {
                if (entityName.find("NotificationServiceAgent") != std::string::npos)
                {
                    fillNSADataWithoutDB();
                    // let the agent know that entity creation is complete
                    m_registeredUser->entityCreationComplete();
    ...
    void GenericAgent::ensureControlMode()
    {
