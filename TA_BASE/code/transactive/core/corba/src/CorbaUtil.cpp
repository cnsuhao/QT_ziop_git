/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/corba/src/CorbaUtil.cpp $
...
namespace TA_Base_Core
{
    ...
    void CorbaUtil::setInstance(CorbaUtil& corbaUtil)
    {
        ...
        CorbaUtil::getInstance().m_activatedPromise.first.set_value();
    }

    CorbaUtil::CorbaUtil()
        : m_orbVar(CORBA::ORB::_nil()), m_running(false), m_activated(false)
    {
        resetActivatePromise();
    }
    ...
    void CorbaUtil::activate()
    {
        ...
        if (! m_activated)
        {
            ...
            try
            {
                ...
                m_activated = true;
                m_activatedPromise.first.set_value();
            }
    ...
    CORBA::Object_ptr CorbaUtil::stringToObject(const std::string& ior)
    {
        ...
        CORBA::Object_var obj = getOrb().string_to_object(ior.c_str());
    ...
    void CorbaUtil::waitForActivate()
    {
        if (!m_activatedPromise.second.is_ready())
        {
            LOG0(SourceInfo, DebugUtil::DebugDebug, "CorbaUtil::waitForActivate()");
            m_activatedPromise.second.wait();
        }
    }

    void CorbaUtil::resetActivatePromise()
    {
        LOG0(SourceInfo, DebugUtil::DebugDebug, "CorbaUtil::resetActivatePromise()");
        m_activatedPromise.first = boost::promise<void>();
        m_activatedPromise.second = m_activatedPromise.first.get_future().share();
    }
    ...
    CORBA::Object_ptr CorbaUtil::enableZIOP(CORBA::Object_ptr obj)
    {
        ...
        {
            LOG_GENERIC(SourceInfo, DebugUtil::DebugDebug, "failed to enable ZIOP: %s", exceptionToString(ex).c_str());
        }
