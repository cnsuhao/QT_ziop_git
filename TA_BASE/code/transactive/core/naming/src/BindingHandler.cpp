          ...
          m_resolving(false)
    {
        ...
        if (m_primaryNSString.empty() && m_secondaryNSString.empty())
        {
            try
            {
                ...
                {
                    m_primaryNSString = str(boost::format("corbaloc::1.2@%s:%d/%sRepo") % m_primaryNSHostname  % m_primaryNSPort % m_agentName);
                }

                if (m_secondaryNSHostname.size() && m_secondaryNSPort != 0)
                {
                    m_secondaryNSString = str(boost::format("corbaloc::1.2@%s:%d/%sRepo") % m_secondaryNSHostname  % m_secondaryNSPort % m_agentName);
                }
            }
