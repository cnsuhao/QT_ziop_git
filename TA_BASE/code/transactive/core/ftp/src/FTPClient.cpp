 ...
 * in any form.
 *
 * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/ftp/src/FTPClient.cpp $
 * @author:  Jade Welton
 * @version: $Revision: #1 $
 *
...
namespace TA_Base_Core
{
	...
	void FTPClient::setPassiveMode( std::string& serverDataAddress, std::string& serverDataPort )
	{
		...
		//
		unsigned int leftBracketPos = passiveModeReplyString.find_last_of( "(" );
		if ( leftBracketPos == std::string::npos )
		{
			TA_THROW( FTPException( "Unable to parse passive mode reply - no left bracket found" ) );
		}
		unsigned int rightBracketPos = passiveModeReplyString.find_first_of( ")", leftBracketPos );
