/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/src/ConfigGui.cpp $
  * @author:  Karen Graham
  * @version: $Revision: #1 $
  *
		  ...
		  m_noType(type.empty())
   {
    ...
    void ConfigGui::setExecutable(const std::string executable)
    {
		...
		if(m_noType)
		{
			...
			int startPos = executable.find(ENTITYNAME);
			
			// it was found..
			if (startPos != std::string::npos)
			{
				int endPos = executable.find(" ", startPos);
    ...
    {
        int pos = executable.find(ENTITYNAME);
