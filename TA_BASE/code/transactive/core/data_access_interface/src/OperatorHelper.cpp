/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/src/OperatorHelper.cpp $
  * @author:  Nick Jardine
  * @version: $Revision: #1 $
  *
...
//				std::string sql  = databaseConnection->prepareSQLStatement(SE_OPERATOR_PROFILE_INSERT_5351, m_key, *iterToAdd);
		SQLStatement sql;
		databaseConnection->prepareSQLStatement(sql, SE_OPERATOR_PROFILE_INSERT_5351, m_key, *iterToAdd);
...
//		TA_ASSERT(sql.size() > 0, "retrieveDeletedField: Get SQL Statement error");
        ...
        if(textToSearchFor.size() + numberString.str().size() > maxFieldSize)
        {
            ...
            unsigned int deletedFlagPos = textToSearchFor.find(DELETED_FLAG);
