...
SQLFileHelper::SQLFileHelper() : m_strSQLFile(defSQLFileName)
{
#if defSQL_MODE_TXT
	_LoadSQLFile();
#endif
...
std::string SQLFileHelper::getSQLString(int nDbType, const std::string& strSQLKey)
{
...
#if defSQL_MODE_TXT
	if (0 == m_SQLOracleHashTable.size() ||
		0 == m_SQLMySQLHashTable.size())
	{
		// should never reach here, but in case of load file
		// failed at system start, reload it once more
		LOG_GENERIC(SourceInfo, DebugUtil::DebugWarn,
			"SQLFileHelper::GetSQLString() The SQL hash table is empty when load SQLID: %s", strSQLKey.c_str());
		_LoadSQLFile();

		TA_ASSERT((0 != m_SQLOracleHashTable.size() && 0 != m_SQLMySQLHashTable.size()), "Load sql.dat file failed");
	}	

	strSql = m_SQLCommonHashTable[strSQLKey];
	if ( !strSql.empty() )
	{
		FUNCTION_EXIT;
		return strSql;
	...
	{
	case enumOracleDb:
		strSql = m_SQLOracleHashTable[strSQLKey];
		break;
	case enumMysqlDb:
		strSql = m_SQLMySQLHashTable[strSQLKey];
		break;
	default:
		strSql = m_SQLCommonHashTable[strSQLKey];
		break;
...
#else
	size_t nValidKey = strSQLKey.find_first_not_of("0123456789");
	TA_ASSERT(nValidKey == std::string::npos, "Invalid SQL Key,");
	int nSQLIndex   = atoi(strSQLKey.c_str());
	int nArrSize = sizeof(s_pzSQLArray) / sizeof (*s_pzSQLArray);
	TA_ASSERT( nSQLIndex >= 0 && nSQLIndex < nArrSize, "Invalid SQL array index...");
...
void  SQLFileHelper::getSQLString(const std::string& strSQLKey, SQLStatement& rSqlStatements)
{
	...
	rSqlStatements.strCommon = s_pzSQLArray[nSQLIndex];
	rSqlStatements.strOracle = s_pzSQLArray[nSQLIndex + enumOracleDb];
	rSqlStatements.strMySQL  = s_pzSQLArray[nSQLIndex + enumMysqlDb];
