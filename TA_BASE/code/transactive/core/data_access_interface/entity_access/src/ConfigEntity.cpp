/**
  ...
  * in any form.
  *
  * Source: $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/entity_access/src/ConfigEntity.cpp $
  * @author Nick Jardine
  * @version $Revision: #1 $
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(ENTITY_V_SELECT_2518, 
    ...
    std::map<std::string, std::string> ConfigEntity::parseEnumerationValues(const std::string& validation)
    {
        ...
        int beginOfFirstPos = validation.find_first_of('[') + 1;
        int endOfFirstPos = validation.find_first_of(']');
        int beginOfSecondPos = validation.find_first_of('[',endOfFirstPos) + 1;
        int endOfSecondPos = validation.find_first_of(']',beginOfSecondPos);
        ...
        int startingPos = 0;                       // This is the start of each string
        int separatorPos = storedValues.find(';'); // This will be the end of each string
