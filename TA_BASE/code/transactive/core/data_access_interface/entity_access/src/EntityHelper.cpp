/**
  ...
  * in any form.
  *
  * Source: $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/entity_access/src/EntityHelper.cpp $
  * @author Nick Jardine
  * @version $Revision: #1 $
  *
...
//					strSql  = databaseConnection->prepareSQLStatement(ENTITYPARAMETER_INSERT_20851, getLocalDatabaseName(), 
		...
		databaseConnection->prepareSQLStatement(strSql, ENTITYPARAMETER_INSERT_20851, getLocalDatabaseName(),
...
//		strSql  = databaseConnection->prepareSQLStatement(ENTITY_INSERT_4801, m_location, PKEY_RANGE_PER_LOCATION, m_location,
		...
		databaseConnection->prepareSQLStatement(strSql, ENTITY_INSERT_4801, m_location, PKEY_RANGE_PER_LOCATION, m_location,
			PKEY_RANGE_PER_LOCATION, databaseConnection->escapeInsertString(m_name),databaseConnection->escapeInsertString(m_address),
...
//		TA_ASSERT(strSql.size() > 0, "EntityHelper::retrieveDeletedNameForEntity(): Prepare SQL Statement error");
        ...
        {
            unsigned long trimLength = (nameToSearchFor.size() + numberString.str().size()) - maxFieldSize;
            unsigned int deletedFlagPos = nameToSearchFor.find(DELETED_FLAG);
...
//		TA_ASSERT(strSql.size() > 0, "EntityHelper::addNewEntityEntityStatusData(): Prepare SQL Statement error");
	...
	int EntityHelper::getIntegerData( const std::string & parameterValue )
	{
		...
		int validCharacters = parameterValue.find_first_not_of("0123456789-");
		int dashLocation = parameterValue.find_last_of("-");
	...
	double EntityHelper::getRealData ( const std::string & parameterValue )
	{
		...
		int validCharacters = parameterValue.find_first_not_of("0123456789-.");				// locates any invalid characters in this string
		...
		int firstDash = parameterValue.find_first_of("-");									// finds the location of the first "-"
		int otherDash = parameterValue.find_first_of("-",firstDash+1);						// determines if there are any subsequent "-"s
		
		if (firstDash != otherDash && -1 != otherDash) // there is more than one dash
		{
		...
		if (-1 != firstDash && 0 != firstDash) // the only dash is not in the first location
		{
		...
		int trailingDash = parameterValue.find_first_of("0123456789",firstDash+1);			// finds the location of the first character that is a number following the dash - this should be firstDash+1
		
		if (-1 != firstDash && (firstDash+1) != trailingDash) // there is not a numerical character immeidately trailing the dash
		{
		...
		int firstPoint = parameterValue.find_first_of(".");									// finds the location of the first "."
		int otherPoint = parameterValue.find_first_of(".",firstPoint+1);					// determines if there are any subsequent "."s
		
		if (firstPoint != otherPoint && -1 != otherPoint) // there is more than one point
		{
		...
		int trailingPoint = parameterValue.find_first_of("0123456789",firstPoint+1);	// finds the locatino of the first -numerical character after the first point - this should return firstPoint+1

		if (-1 != firstPoint && (firstPoint+1) != trailingPoint) // there is not a numerical character immeidately trailing the point
		{
		...
		if (-1 != firstPoint && ((0 == firstPoint) || (-1 != firstDash && 1 == firstPoint))) // the decimal point is in an ivalid location (either the first position, or the second if there is a dash)
		{
