/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/entity_access/src/RadioGlobalEntityData.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
    ...
    std::vector<RadioGlobalEntityData::RadioPriority> RadioGlobalEntityData::getRadioPriorities(std::string parameterValue)
    {
        ...
        if ( (commaParts.size() > 0) && (commaParts[0] != "") )
        {
                 ...
                 commaIter  != commaParts.end(); commaIter++)
            {
                ...
                if (item.find("-") != -1)
                {
                    int dashPos = item.find_first_of("-", 0 );

                    int first = atoi(item.substr(0,dashPos).c_str());
                    int last = atoi(item.substr(dashPos + 1, item.length()).c_str());
