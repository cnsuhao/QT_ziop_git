/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/user_settings/src/UserSettingsData.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
//		std::string strSql  = databaseConnection->prepareSQLStatement(USER_SETTINGS_INSERT_40801, m_sessionId,
    ...
    std::string UserSettingsData::unsignedLongToString(unsigned long x)
    {
        ...
        char buffer[12]; //just to be safe
        sprintf(buffer,"%u",x);
    ...
    unsigned long UserSettingsData::stringToUnsignedLong(const std::string& str)
    {
		...
		int validCharacters = str.find_first_not_of("0123456789");
