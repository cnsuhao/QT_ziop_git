/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/data_access_interface/interlocking/src/InterlockAlias.cpp $
  * @author:  Ross Tucker
  * @version: $Revision: #1 $
  *
...
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), LK_ALIAS_STD_SELECT_31001, m_ruleId, 
//		std::string strSql  = databaseConnection->prepareSQLStatement(LK_ALIAS_SELECT_31001, m_ruleId, 
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, LK_ALIAS_SELECT_31001, m_ruleId,
			databaseConnection->escapeQueryString(m_variableName) );
    ...
    {       
        ...
        {
            TA_THROW(TA_Base_Core::DataConfigurationException("Interlocking alias data not fully specified. Interlocking alias cannot be written to database", fieldNames));
        }
...
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), LK_ALIAS_STD_UPDATE_31151,
//		std::string strSql  = databaseConnection->prepareSQLStatement(LK_ALIAS_UPDATE_31151,
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, LK_ALIAS_UPDATE_31151,
			databaseConnection->escapeInsertString(m_alias), databaseConnection->escapeInsertString(m_variableType),
			m_ruleId, databaseConnection->escapeInsertString(m_variableName) );
    ...
    void InterlockAlias::addNewInterlockAlias()
    {
        ...
        sql << ")";*/
//		std::string strSql = defPrepareSQLStatement(databaseConnection->GetDbServerType(), LK_ALIAS_STD_INSERT_31301, m_ruleId,
//		std::string strSql  = databaseConnection->prepareSQLStatement(LK_ALIAS_INSERT_31301, m_ruleId,
		SQLStatement strSql;
		databaseConnection->prepareSQLStatement(strSql, LK_ALIAS_INSERT_31301, m_ruleId,
			databaseConnection->escapeInsertString(m_alias), databaseConnection->escapeInsertString(m_variableName),
			databaseConnection->escapeInsertString(m_variableType) );
