/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/configuration_updates/src/OnlineUpdateListener.cpp $
  * @author:  San Teo
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
...

        {
            ...
            if (m_interestedObject.find(objectType) == m_interestedObject.end())
            {
                ...
                LOG2(SourceInfo, DebugUtil::DebugInfo,
                     "Registering interest for the first time. Added object type = <%s> and keys = <%s>",
                     getDescFromEObjectType(objectType).c_str(), keysStr.str().c_str());
            ...
            for (size_t i = 0; i < key.size(); i++)
            {
                if (keyCallback.find(key[i]) != keyCallback.end())
                {
                    ...
                    TA_ASSERT(keyCallback[key[i]] == &callbackObj,  "Object type registered with different callback object");
                }
                else
                {
                ...
                LOG2(SourceInfo, DebugUtil::DebugInfo,
                     "Registering different interest for same object. Added object type = <%s> and keys = <%s>",
                     getDescFromEObjectType(objectType).c_str(), keysStr.str().c_str());
            }
    ...
    void OnlineUpdateListener::registerAllInterests(
        ...
        IOnlineUpdatable& callbackObj)
    {
        LOG(SourceInfo, DebugUtil::FunctionEntry, "registerAllInterests");

        {
            ...
            if (m_interestedObject.find(objectType) == m_interestedObject.end())
            {
                ...
                LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugInfo,
                    "Registering to listen to all updates of object type = %s", getDescFromEObjectType(objectType).c_str());
                LOG(SourceInfo, DebugUtil::FunctionExit, "registerAllInterests");
                return;
            }
        ...
        LOG(SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugDebug, "Registered to listen to all updates");
        LOG(SourceInfo, DebugUtil::FunctionExit, "registerAllInterests");
    }
