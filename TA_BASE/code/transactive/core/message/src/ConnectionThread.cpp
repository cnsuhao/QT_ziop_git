/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/message/src/ConnectionThread.cpp $
...
namespace TA_Base_Core
{
    ...
    std::string getChannelLocatorAddr(const std::string& serviceAddr)
    {
        ...
        if (serviceAddr.find(":") == serviceAddr.npos)
        {
            ...
            sprintf(channelLocatorAddress, "corbaloc::1.2@%s:%d/%s", serviceAddr.c_str(),
        ...
        else
        {
            ...
            sprintf(channelLocatorAddress, "corbaloc::1.2@%s/%s", serviceAddr.c_str(),
                    TA_Base_Core::IChannelLocator::DEFAULT_LOCATOR_NAME);
        }
