/**
  ...
  * in any form.
  *
  * Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/process_management/src/UtilityInitialiser.cpp $
  * @author:  Ripple
  * @version: $Revision: #1 $
  *
...
namespace TA_Base_Core
{
    ...
    bool UtilityInitialiser::initialiseUtilities(int argc, char* argv[])
    {
        try
        {
            ...
            //xinsong++ first read parameter, then decide to set the RPARAM_NSHOSTNAME or not
            // yanrong++: CL-21230
            // set hostname run parameter
            TA_Base_Core::RunParams::getInstance().set(RPARAM_NSHOSTNAME, TA_Base_Core::Hostname::getHostname().c_str());
    ...
    void UtilityInitialiser::initialiseCorba()
    {
        ...
        if (false == CorbaUtil::getInstance().initialise(port)) // yanrong++: CL-21230
        {
