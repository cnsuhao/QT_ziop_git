...
* in any form.
*
* Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/database/src/SimpleDb.cpp $
* @author:  Ripple
* @version: $Revision: #1 $
*
...
namespace TA_Base_Core
{
    ...
    std::vector<std::string> SimpleDb::ParseConnectStr(const std::string& str, char delimiter)
    {
        std::vector<std::string> components;
        ParseConnectStr(str, components, delimiter);
        return components;
    }
    ...
    std::string SimpleDb::getDbNameFromConnectStr(const std::string& str)
    {
        std::vector<std::string> components;
        ParseConnectStr(str, components);
        std::string dbName = RPARAM_DBPREFIX + (components[1]);

        if (4 < components.size() && !components[4].empty())
        {
            return dbName + "@" + components[4];
        }
