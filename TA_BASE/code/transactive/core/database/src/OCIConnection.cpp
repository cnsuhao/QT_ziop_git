...
* in any form.
*
* Source:   $File: //depot/PRIV_GenericGUI_QT/TA_BASE/transactive/core/database/src/OCIConnection.cpp $
* @author:  Ripple
* @version: $Revision: #1 $
*
...
namespace TA_Base_Core
{
	...
	*/
	...
	void OCIConnection::exec( const SQLStatement& rSqlObj, IQueryAdapter*& pIQueryAdapter, bool isQuery /*= false*/, int prefetch /*= 10*/)
	{
		...
		{
			LOG( SourceInfo, DebugUtil::GenericLog, DebugUtil::DebugError, "Failed to exec SQLID: %d, SQL: %s", rSqlObj.nSQLID, strSql.c_str() );
			DBException db("OCI_ERROR", e.message().c_str(), e.errorCode());
